...
router.get('/anweisungen-wissen', ensureAuthenticated, async (req, res) => {
  try {
    ...
    console.error('[Auth Routes /anweisungen-wissen GET] Error:', error.message);
    res.status(500).json({ success: false, message: 'Fehler beim Laden der Daten', details: error.message });
  }
});
...
router.put('/anweisungen-wissen', ensureAuthenticated, async (req, res) => {
  try {
    ...
    console.log('[Auth Routes /anweisungen-wissen PUT] Incoming request body for user:', userId);
    ...
    console.log('[Auth Routes /anweisungen-wissen PUT] Prepared profile payload:');
    ...
    console.log(`[Auth Routes /anweisungen-wissen PUT] Deleting knowledge entries for user ${userId} NOT in this list of IDs: [${submittedIds.join(',')}]`);
    ...
    if (validEntries.length > 0) {
        ...
        if (toUpdate.length > 0) {
            console.log('[Auth Routes /anweisungen-wissen PUT] Updating knowledge entries:');
        ...
        if (toInsert.length > 0) {
            console.log('[Auth Routes /anweisungen-wissen PUT] Inserting new knowledge entries:');
    ...
    res.json({ success: true, message: 'Anweisungen & Wissen gespeichert' });
  } catch (error) {
    console.error('[Auth Routes /anweisungen-wissen PUT] Error:', error.message);
    res.status(500).json({ success: false, message: 'Fehler beim Speichern', details: error.message });
  }
});
...
router.delete('/anweisungen-wissen/:id', ensureAuthenticated, async (req, res) => {
  try {
    ...
    if (!id) {
      return res.status(400).json({ success: false, message: 'Keine ID angegeben.' });
    }
    ...
    res.json({ success: true, message: 'Wissenseintrag gelöscht.' });
  } catch (error) {
    console.error(`[Auth Routes /anweisungen-wissen/${req.params.id} DELETE] Error:`, error.message);
    res.status(500).json({ success: false, message: 'Fehler beim Löschen des Eintrags.', details: error.message });
  }
});
// === END PERSONAL INSTRUCTIONS & KNOWLEDGE ENDPOINTS ===
