...
- `minimumReleaseAge: 10080` (7 Tage; pnpm 11 setzt 1440 als Default, 10.x gar nichts) — schließt das Fenster, in dem kompromittierte Versionen typischerweise erkannt und zurückgezogen werden (Shai-Hulud/`chalk`-Vorfälle 2025 lagen jeweils unter 48 h). Ausnahmeliste `minimumReleaseAgeExclude` für unsere eigenen Hotfix-Pfade (`@assistant-ui/*`, `@blocknote/*`), wenn ein Bugfix nicht warten kann. **Seit pnpm 12 prüft jeder Install das ganze Lockfile** gegen die Schranke, `--frozen-lockfile` in CI und im Image-Build eingeschlossen — gemessen am 23.09.2026 mit 12.4.2, Schranke testweise auf 60 Tage: frozen und `--lockfile-only` brechen beide mit `ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION` ab, das Lockfile bleibt unverändert (kein stilles Zurückstufen); mit der echten Schranke läuft die Neuauflösung durch. Ein junger Eintrag kommt nur über die Ausnahmeliste hinein. Der Sieben-Tage-Stillstand aus #3216 (pnpm 10 lehnte bei jeder Neuauflösung die eigenen, noch vor der Schranke gelockten Einträge ab) entsteht damit nur noch, wenn ein Lockfile an der Schranke vorbei geschrieben wurde — und das fällt jetzt schon in CI auf, statt erst beim nächsten Bump.
...
- `trustPolicy: no-downgrade` (ab pnpm 10.21): Install bricht ab, wenn eine neue Version eines Pakets ein **niedrigeres Vertrauensniveau** hat als die vorige (z. B. Wechsel von Trusted Publishing zurück auf ein Token) — genau das Muster übernommener Maintainer-Konten. Verglichen wird nach **Veröffentlichungsdatum, nicht nach semver**, und der Lauf bricht beim **ersten** Treffer ab: lang gesetzte Backports (`semver@6.3.1`, `fbjs@3.0.5`, `ua-parser-js@1.0.41`, `tailwind-merge@2.6.0`) lesen sich damit als Übernahme und blockierten die Lock-Neuauflösung vollständig (#3247, #3279, #3283). Die Ausnahme ist deshalb **`trustPolicyIgnoreAfter: 525600`** — Einheit **Minuten**, also ein Jahr Prüffenster; in Millisekunden gelesen wären es 8,75 Minuten und die Prüfung faktisch aus. **Nicht** `trustPolicyExclude`: das gilt nur je Paket, und weil der Abbruch beim ersten Treffer sitzt, holt jede Ausnahme die nächste hervor, und jeder Eintrag bleibt ein dauerhaftes Loch für dieses Paket (PR #3309).
