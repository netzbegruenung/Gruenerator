  ...
  return (
    <div className="pointer-events-none absolute top-0 bottom-0 left-0">
      ...
      {Array.from({ length: level }, (_, index) => {
        ...
        return (
          <div
            className="absolute top-0 bottom-0 border-grey-300/40 dark:border-grey-600/40 border-l"
            key={index.toString()}
            style={{
      ...
      <div
        className="absolute top-1/2 border-grey-300/40 dark:border-grey-600/40 border-t"
        style={{
      ...
      {isLast && (
        <div
          className="absolute top-0 border-grey-300/40 dark:border-grey-600/40 border-l"
          style={{
            left: (level - 1) * (indent ?? 0) + 12,
            height: '50%',
          }}
...
export const TreeExpander = ({
  ...
  return (
    <motion.div
      ...
      {...props}
    >
      <FiChevronRight className="h-3 w-3 text-grey-400 dark:text-grey-500" />
    </motion.div>
  );
};
...
export const TreeIcon = ({ icon, hasChildren = false, className, ...props }: TreeIconProps) => {
  ...
  const getDefaultIcon = () =>
    hasChildren ? <FiFolder className="h-4 w-4" /> : <FiFile className="h-4 w-4" />;

  return (
    <motion.div
      className={cn(
        'mr-2 flex h-4 w-4 items-center justify-center text-grey-400 dark:text-grey-500',
        className
      )}
