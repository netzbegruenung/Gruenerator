...
export const Schema = z.object({
  ...
  objectiveTitle: z
    ...
    .default('Accelerate enterprise customer acquisition across EMEA and North America'),
  description: z
    .string()
    .max(300)
  ...
  kpis: z
    .array(
      z.object({
        ...
        progressPercentage: z.number().min(0).max(100).describe('Progress percentage value'),
...
const SemiCircleProgress = ({ percentage, color }: { percentage: number; color: string }) => {
  ...
  return (
    <div className="relative w-[150px] h-[75px] overflow-hidden">
      <svg viewBox="0 0 100 50" className="w-full h-full transform transition-all duration-500">
        <path
    ...
    <div className="relative min-w-[300px] ">
      ...
      >
        ...
        <div className=" w-full h-[135px] flex items-center px-6">
          ...
          <SemiCircleProgress percentage={kpi.progressPercentage} color={kpi.color} />
        </div>
      </div>
    </div>
  );
          ...
          <div className="flex gap-[18px] w-full items-center justify-center">
            {data.kpis.length > 2 && (
              <div className="flex flex-col gap-[18px]">
                <div className="">{data.kpis[3] && <KPICard kpi={data.kpis[3]} />}</div>
                <div className=" ">{data.kpis[4] && <KPICard kpi={data.kpis[4]} />}</div>
              </div>
            )}
            <div className="flex flex-col gap-[18px]">
              <div className=" ">{data.kpis[0] && <KPICard kpi={data.kpis[0]} />}</div>
              <div className=" ">{data.kpis[1] && <KPICard kpi={data.kpis[1]} />}</div>
              <div className="">{data.kpis[2] && <KPICard kpi={data.kpis[2]} />}</div>
            </div>
          </div>
        </div>
      </div>
