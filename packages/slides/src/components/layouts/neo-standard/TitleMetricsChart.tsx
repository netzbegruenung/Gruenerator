        ...
        return (
          <ResponsiveContainer width="100%" height={400}>
            ...
            >
              ...
              {hasValue2 && (
                <Bar dataKey="value2" fill="var(--graph-1, #A8D9A8)" radius={[0, 4, 4, 0]} />
              )}
            </BarChart>
          </ResponsiveContainer>
        );
          ...
          <div className="flex-grow relative">
            {chartType !== 'pie' && chartType !== 'donut' && (
              <div className="flex justify-center gap-[30px] mb-[20px]">
                {chart?.seriesNames
                  ?.slice(0, chart?.data?.some((row) => (row.value2 ?? 0) > 0) ? 2 : 1)
                  ?.map((name, i) => (
                    <div key={i} className="flex items-center gap-[8px]">
                      <div
                        className={`w-[12px] h-[12px] rounded-full ${i === 0 ? 'bg-[#1F8A2E]' : 'bg-[#A8D9A8]'}`}
                        style={{
                          backgroundColor:
                            i === 0 ? 'var(--graph-0, #1F8A2E)' : 'var(--graph-1, #A8D9A8)',
                        }}
                      />
                      <span
