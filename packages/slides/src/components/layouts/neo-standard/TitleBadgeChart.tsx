        ...
        .array(
          z.object({
            ...
            value2: z
              .number()
              .optional()
              .describe('Value for the second series (optional for single-series charts)'),
          })
        )
        .max(4)
...
const dynamicSlideLayout: React.FC<{ data: Partial<z.infer<typeof Schema>> }> = ({ data }) => {
          ...
          <ResponsiveContainer width="100%" height={400}>
            ...
            >
              ...
              <Legend
                ...
                formatter={(value, entry, index) => graphData?.columns?.[index + 1] || value}
              />
              <Line
                type="monotone"
                dataKey="value1"
        ...
        return (
          <ResponsiveContainer width="100%" height={400}>
            ...
            >
              ...
              {hasValue2 && (
                <Bar dataKey="value2" fill="var(--graph-1, #A7DBA8)" radius={[0, 4, 4, 0]} />
              )}
            </BarChart>
          </ResponsiveContainer>
        );
          ...
          <ResponsiveContainer width="100%" height={400}>
            <PieChart margin={{ top: 20, right: 20, left: 20, bottom: 20 }}>
              <Pie
                ...
                labelLine={{ stroke: 'var(--background-text, #333333)', strokeWidth: 1 }}
              >
                {pieData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={`var(--graph-${index}, ${CHART_COLORS[index % CHART_COLORS.length]})`}
                  />
                ))}
              </Pie>
              <Tooltip cursor={{ fill: 'transparent' }} />
  ...
  return (
    <>
      ...
      />
      <div
        className="relative w-full rounded-sm max-w-[1280px] shadow-lg h-[720px] aspect-video bg-white z-20 mx-auto overflow-hidden  p-8 box-border "
        style={{
      ...
      >
        ...
        {/* Main Content Area */}
        <div
          className=" flex-grow flex flex-col relative z-10 border-[1.3px] border-[#F0F0F2] rounded-[10px] p-6 pb-8 "
        ...
        >
          ...
          {/* Chart Area */}
          <div className=" flex-grow w-full min-h-0 ">{renderChart()}</div>
