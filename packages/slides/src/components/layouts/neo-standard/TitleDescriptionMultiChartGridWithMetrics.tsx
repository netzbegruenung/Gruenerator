          ...
          >
            ...
            {series.map((s: string, index: number) => (
              <Bar
                ...
                stackId="stack"
                fill={graphColors(index)}
                radius={index === series.length - 1 ? [0, 3, 3, 0] : [0, 0, 0, 0]}
              />
            ))}
          </BarChart>
        </ResponsiveContainer>
...
const TitleDescriptionMultiChartGridWithMetricsLayout: React.FC<MultiChartGridSlideLayoutProps> = ({
  data: slideData,
}) => {
  const title = slideData?.title || 'Data Analytics Dashboard';
  const description =
    slideData?.description || 'Comprehensive overview of key metrics and performance indicators.';
    ...
    <>
      ...
      >
        ...
        <div className="relative w-full h-full p-[60px] pt-[50px] flex flex-col">
          ...
          <div
            className={`flex-1 grid ${gridLayout.className} gap-4 min-h-0`}
            style={{ height: hasMetrics ? '450px' : 'calc(100% - 140px)' }}
          >
            {charts.map((chart, index) => (
              ...
              >
                ...
                <div
                  className="flex-1 px-2 pb-2 min-h-0"
