              ...
              .array(
                z.object({
                  ...
                  value2: z
                    .number()
                    .optional()
                    .describe('The second value (optional for single-series charts)'),
                })
              )
              .max(4)
...
const dynamicSlideLayout: React.FC<{ data: Partial<z.infer<typeof Schema>> }> = ({ data }) => {
  ...
  const renderChart = (
    section: typeof comparisonSections extends (infer T)[] | undefined ? T : never
  ) => {
        ...
        return (
          <ResponsiveContainer width="100%" height={380}>
            ...
            >
              ...
              {hasValue2 && (
                <Bar dataKey="value2" fill="var(--graph-1, #A9D9B1)" radius={[0, 4, 4, 0]} />
              )}
            </BarChart>
          </ResponsiveContainer>
        );
      ...
      >
        ...
        <div className="flex justify-center gap-[25px] px-[36px]">
          {comparisonSections?.map((section, idx) => (
            <div
              key={idx}
              className="relative flex flex-col w-[590px] min-h-[500px] border-[1.3px]  rounded-[6px] p-6"
            ...
            >
              ...
              {section?.chartType !== 'pie' && section?.chartType !== 'donut' && (
                <div
                  className={`flex justify-center items-center gap-6 mb-4 ${!section?.tagline ? 'mt-8' : ''}`}
                >
                  {section?.legendItems
                    ?.slice(0, section?.graph?.rows?.some((row) => (row.value2 ?? 0) > 0) ? 2 : 1)
                    ?.map((item, lIdx) => (
                      <div key={lIdx} className="flex items-center gap-2">
                        <div
                          className={`w-3 h-3 rounded-full ${lIdx === 0 ? 'bg-var(--graph-0, #1F8A2E)' : 'bg-var(--graph-1, #A9D9B1)'}`}
                        />
                        <span
              ...
              {/* Chart Area */}
              <div
                className={`flex-grow w-full h-[300px] ${(section?.chartType === 'pie' || section?.chartType === 'donut') && !section?.tagline ? 'mt-8' : ''}`}
              >
                {renderChart(section)}
              </div>
