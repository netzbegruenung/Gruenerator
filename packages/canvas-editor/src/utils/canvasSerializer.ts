 ...
 */
export function serializeCanvasState(config: CanvasConfig, state: CanvasState): FreeCanvasRequest {
  ...
  const balkens = state.balkenInstances?.map(
    (balken: Record<string, unknown>) =>
      ({
        id: String(balken.id),
  ...
  const illustrations = state.illustrationInstances?.map(
    (illust: Record<string, unknown>) =>
      ({
        id: String(illust.id),
  ...
  const shapes = state.shapeInstances?.map(
  ...
  const icons = state.iconInstances?.map(
  ...
  const texts = state.textInstances?.map(
