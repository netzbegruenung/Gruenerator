/**
 * Presentation design tokens — Grundlagendesign 2025
 *
 * Pure data (colors, layout constants) extracted from the official B90/Grüne
 * PowerPoint template. Kept as a dependency-free leaf module (only imports the
 * leaf `canvasAssets`) so BOTH the configs and the sidebar can import it
 * one-directionally — neither side forms a cross-chunk init cycle. The old
 * `configs/presentation/presentationTheme` path re-exports from here for
