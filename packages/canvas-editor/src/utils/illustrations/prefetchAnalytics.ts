...
// Service worker functions - no-op fallbacks when not available (e.g., mobile)
const getCacheSize = async (): Promise<{ cacheSize?: number; cacheName?: string }> => ({});
const isServiceWorkerActive = async (): Promise<boolean> => false;
