...
export interface PageState<TState = Record<string, unknown>> {
  id: string;
  state: TState;
  order: number;
}
...
export function useMultiPageCanvas<TState extends Record<string, unknown>, TActions>({
  config,
  initialProps,
  maxPages = 10,
  ...
  const addPage = useCallback(() => {
    ...
    // Build background state - handle both image AND solid color backgrounds
    const backgroundState: Record<string, unknown> = {};

    // Copy image background if present
    if (bgImage) {
      backgroundState.imageSrc = bgImage;
      backgroundState.currentImageSrc = bgImage;
    }

    // Copy solid color background if present
    if (lastState.backgroundColor) {
      backgroundState.backgroundColor = lastState.backgroundColor;
    }

    const newPageState = config.createInitialState({
      ...(config.multiPage?.defaultNewPageState || {}),
      // Copy background from previous page (both image and color)
      ...backgroundState,
    }) as TState;
