/**
 * Vendored from the assistant-ui Elements registry (@assistant-ui/elements-surfaces),
 * the shared token layer every Element imports. Adopted so that future elements
 ...
 *   2. `ShimmerLabel` uses this repo own .shimmer-text (styles/chat.css)
...
export const collapsePanel =
  'overflow-hidden transition-[height] duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] data-[state=closed]:h-0 data-[state=open]:h-[var(--radix-collapsible-content-height)] motion-reduce:transition-none';

export const live = 'text-primary dark:text-primary-400';
...
export function ShimmerLabel({
  ...
  return (
    <span
      className={cn(active && 'shimmer-text motion-reduce:animate-none', className)}
      {...props}
    />
  );
}
