    ...
    async (query: string) => {
      ...
      try {
        ...
        if (searchData.status === 'success' && Array.isArray(searchData.results)) {
          ...
          try {
            const analysisResponse = await client.post<AnalysisResponse>(SEARCH_ENDPOINTS.ANALYZE, {
  ...
  const deepSearch = useCallback(
    async (query: string) => {
      ...
      try {
        ...
        const response = await client.post<DeepSearchResponse>(SEARCH_ENDPOINTS.DEEP_RESEARCH, {
          query,
        });
  ...
  const webSearch = useCallback(
    async (query: string) => {
      ...
      try {
        ...
        const response = await client.post<WebSearchResponse>(SEARCH_ENDPOINTS.WEB_SEARCH, {
          query,
          searchType: 'general',
          includeSummary: true,
          maxResults: 10,
