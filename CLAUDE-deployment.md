...
Thumbs up/down in the chat UI post to `POST /api/chat-service/feedback`, which writes a `user-feedback` score onto the turn's trace. The buttons render only when the `done` event carried a `traceId`, i.e. only when Langfuse is actually on. Set the project's **data retention to 30 days** in the Langfuse UI to match the Datenschutzerklärung. Init lives in `apps/api/instrument.ts` (runs in every cluster worker via `--import`); worker-thread LLM calls (classifier etc.) are not traced.
