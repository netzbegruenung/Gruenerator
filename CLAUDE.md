...
- **`apps/api`** — Express 5, Node.js cluster mode. AI runs in-process via `services/ai/aiService.ts`, reachable as `app.locals.aiClient` (typed helper `utils/getAiClient.ts`); provider execution sits in `services/ai/execution/`. The `worker_threads` pool both replaced is gone. Routes in `routes/`, logic in `services/`. See `docs/CLAUDE-routing.md`.
  - **Zwei Wege zum Modell, und der neue ist der Zielzustand.** `aiClient.processRequest` nimmt den alten, untypisierten Envelope (`type` als bare string, OpenAI-Drahtnamen wie `max_tokens`, Ergebnis `{content, success}`) — gemessen am 16.08.2026 noch **62 Aufrufe in 55 Produktionsdateien**. `services/ai/generate.ts` (`aiText`/`aiObject`/`aiTools`) ist die typisierte Fassade auf derselben Maschine: `executeProvider` ruft, geroutet über die Tabelle in `services/ai/lanes.ts`. **Neuer Code nimmt die Fassade.** Beide klassifizieren Provider-Fehler zu `AiProviderError` (`processRequest` an seiner Grenze, die Fassade über `NoAnswerError`) — wer eine dritte Tür baut, muss das mitbauen, sonst kommt jeder Ausfall als nacktes `internal` beim Client an. Zwei Fallen beim Umzug einer Aufrufstelle: ein `type` OHNE Zeile in `AI_LANES` (z. B. `chat_intent_classification`, das seinen Provider/Modell aus `intermediateLanes.ts` pinnt) landet auf `default` und wird als Versehen protokolliert; und `response_format: {type:'json_object'}` heißt auf der Fassade `json: true` — weglassen macht erzwungenes JSON still wieder zu einer Prompt-Bitte.
