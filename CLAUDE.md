...
**Der Haken, der die Reparatur kostet: `pnpm install` merkt das nicht.** pnpm 10 cacht den Workspace-Zustand in `node_modules/.pnpm-workspace-state.json` und meldet danach „Already up to date" in ~130 ms — auch mit `--force`, und auch wenn die Pakete **physisch gelöscht** sind (am 17.09.2026 gemessen; geschrieben hatte den Zustand ein `pnpm@12.4.2`, während das Repo `pnpm@10.0.0` pinnt). Wer den Baum repariert, löscht erst diese Datei:
