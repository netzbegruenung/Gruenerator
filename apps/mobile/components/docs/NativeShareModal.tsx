...
const SHARE_MODE_CONFIG: Array<{
  ...
  { mode: 'private', icon: 'lock-closed', label: 'Privat', desc: 'Nur eingeladene Personen' },
  {
    ...
    icon: 'people',
    label: 'Mit Anmeldung',
    desc: 'Alle angemeldeten Nutzer*innen',
  },
...
function PermissionsSheet({
  ...
  const updateMode = async (mode: ShareMode) => {
    try {
      await docsShareApi.updateShareMode(documentId, mode);
      setShareMode(mode);
    } catch {
      /* silent */
    }
  ...
  const togglePermission = async () => {
    ...
    try {
      await docsShareApi.updateSharePermission(documentId, next);
  ...
  const removeCollab = async (userId: string) => {
    try {
      await docsShareApi.removeCollaborator(documentId, userId);
  ...
  const removeGroup = async (groupId: string) => {
    try {
      await docsShareApi.unshareFromGroup(documentId, groupId);
      setGroupCollabs((p) => p.filter((g) => g.group_id !== groupId));
    } catch {
      /* silent */
    }
  };
    ...
    <BottomSheet visible={visible} onClose={onClose}>
      ...
      {loading ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
            ...
            {SHARE_MODE_CONFIG.map((opt) => (
              ...
              >
                <Ionicons
                  name={opt.icon}
                  size={20}
                  color={shareMode === opt.mode ? colors.primary[600] : theme.textSecondary}
                />
                <View style={styles.permOptionContent}>
                  <Text
                    style={[
                      styles.permOptionLabel,
                      { color: shareMode === opt.mode ? colors.primary[600] : theme.text },
                    ]}
                  >
                    {opt.label}
                  </Text>
                  <Text style={[styles.permOptionDesc, { color: theme.textSecondary }]}>
                    {opt.desc}
                  </Text>
                </View>
                {shareMode === opt.mode && (
          ...
          {shareMode !== 'private' && (
            <View style={styles.permSection}>
              <Text style={[styles.permSectionLabel, { color: theme.textSecondary }]}>
                Link-Berechtigung
              </Text>
              <TouchableOpacity
              ...
              {collaborators.map((c) => (
                <View key={c.user_id} style={styles.permRow}>
                  <View style={[styles.permAvatar, { backgroundColor: colors.primary[100] }]}>
                    <Text style={styles.permAvatarText}>
                      {(c.display_name || c.email).slice(0, 2).toUpperCase()}
                    </Text>
                  </View>
                  <View style={styles.permRowContent}>
                    <Text style={[styles.permRowName, { color: theme.text }]} numberOfLines={1}>
                      {c.display_name || c.email}
                    </Text>
                    <Text style={[styles.permRowSub, { color: theme.textSecondary }]}>
                      {PERMISSION_LABELS[c.permission_level]}
                    </Text>
                  </View>
              ...
              {groupCollabs.map((g) => (
                <View key={g.group_id} style={styles.permRow}>
                  <Ionicons
                    ...
                    style={styles.permGroupIcon}
                  />
                  <Text
                    style={[styles.permRowName, { color: theme.text, flex: 1 }]}
                    numberOfLines={1}
                  >
                    {g.group_name}
                  </Text>
...
export function NativeShareModal({
  ...
  const handleTextFormatExport = useCallback(
    ...
    <>
      ...
      <BottomSheet visible={visible} onClose={onClose}>
        <View style={styles.grid}>
          <QuickAction
            icon={copiedLink ? 'checkmark-circle' : 'link'}
            label={copiedLink ? 'Kopiert!' : 'Link kopieren'}
      ...
      <BottomSheet visible={showDownload} onClose={() => setShowDownload(false)}>
        ...
        >
          ...
          <View style={styles.downloadRowContent}>
            ...
            <Text style={[styles.downloadRowSub, { color: theme.textSecondary }]}>
              .docx — Microsoft Word
            </Text>
          </View>
