...
import { useNotebookChatStore, type NotebookChatMessage, type NotebookSource } from '../stores/notebookChatStore';
...
export function useNotebookChat({ notebookId }: UseNotebookChatOptions): UseNotebookChatReturn {
  ...
  const { getMessages, addMessage, updateLastMessage, clearMessages, loadFromStorage } = useNotebookChatStore();
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      if (config.collectionType === 'multi') {
        ...
        for (const [collectionId, collectionSources] of Object.entries(response.sourcesByCollection || {})) {
