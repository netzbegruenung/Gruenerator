/**
 * Layout Planner Agent for Text2Sharepic
 *
 * AI-powered agent that analyzes user descriptions and generates
...
function analyzeDescription(description) {
  ...
  let detectedCategory = 'statement'; // default
  ...
  const wantsImage = lowerDesc.includes('bild') || lowerDesc.includes('foto') ||
                     lowerDesc.includes('image') || lowerDesc.includes('hintergrund');
 ...
 */
function generateColorScheme(mood, wantsImage, wantsGradient) {
 ...
 */
function generateZoneConfig(zone, analysis, colorScheme, content) {
  const config = {
    zoneName: zone.name,
    component: null,
    params: {}
  ...
  if (zoneName === 'main-text' || zoneName === 'center-area' ||
      zoneName === 'text-area' || zoneName === 'left-content' || zoneName === 'text-block') {
  ...
  if (zoneName === 'balken-group') {
    if (zone.allowedComponents.includes('text-balken-group')) {
      ...
      const lines = content.lines && content.lines.length > 0
        ? content.lines
        : [content.line1 || 'Zeile 1', content.line2 || 'Zeile 2', content.line3 || 'Zeile 3'];
  ...
  if (zoneName === 'line1' || zoneName === 'line2' || zoneName === 'line3') {
    ...
    const lineText = content.lines?.[lineIndex] || content[`line${lineIndex + 1}`] || `Zeile ${lineIndex + 1}`;

    // Use text-balken component for proper dreizeilen style
    config.component = 'text-balken';
    config.params = {
      text: lineText,
      backgroundColor: lineIndex % 2 === 0 ? CORPORATE_DESIGN.colors.tanne : CORPORATE_DESIGN.colors.sand,
      textColor: lineIndex % 2 === 0 ? CORPORATE_DESIGN.colors.sand : CORPORATE_DESIGN.colors.tanne,
      fontSize: 85,
      font: 'GrueneTypeNeue',
      angle: 12,
      paddingFactor: 0.3,
 ...
 */
function buildLayoutPrompt(description, templates, components) {
 ...
 */
async function generateLayoutPlan(description, options = {}) {
  ...
  const layoutPlan = {
    ...
    colorScheme,
    content,
    zones: zoneConfigs.filter(z => z.component !== null),
    metadata: {
 ...
 */
function extractContentFromDescription(description, category) {
  const content = {
    mainText: '',
    headline: '',
    body: '',
  ...
  if (category === 'event') {
    const dateMatch = description.match(/(\d{1,2}\.\s*(?:januar|februar|märz|april|mai|juni|juli|august|september|oktober|november|dezember|\d{1,2})\.?\s*(?:\d{4})?)/i);
  ...
  if (!content.mainText && !content.quote) {
    ...
    const phraseMatch = description.match(/(?:erstelle|mache|generiere)[^:]*?(?:für|zum|über|mit)?\s*[:\s]*([A-ZÄÖÜ][^.!?\n]{10,})/i);
    if (phraseMatch) {
 ...
 */
async function generateLayoutPlanWithAI(description, claudeApiHelper, options = {}) {
  ...
  try {
    ...
    const response = await claudeApiHelper.callClaude({
      systemPrompt: 'Du bist ein Experte für visuelles Design und Sharepic-Erstellung. Antworte immer mit validem JSON.',
      userMessage: prompt,
      options: {
        temperature: 0.7,
        maxTokens: 2000
    ...
    console.error('[LayoutPlanner] AI generation failed, falling back to rule-based:', error.message);
  }
 ...
 */
function validateLayoutPlan(plan) {
  const errors = [];
