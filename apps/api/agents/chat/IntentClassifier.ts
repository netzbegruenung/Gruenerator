...
export const AGENT_MAPPINGS: AgentMappings = {
  ...
  imagine: {
    ...
    description:
      'AI image generation with FLUX - pure images, sharepics with titles, or image transformation',
    params: {},
  },
};
      ...
      if (aiResult && aiResult.intents && aiResult.intents.length > 0) {
        ...
        if (requestType === 'conversation') {
          ...
          return {
            isMultiIntent: false,
            intents: [
              { agent: 'conversation', route: 'conversation', params: {}, confidence: 0.9 },
            ],
            method: 'ai',
            confidence: 0.9,
...
WICHTIG: editContext ist NUR bei requestType="text_edit" erforderlich!${context.singleIntentOnly ? '\n\nWICHTIG: Gib NUR EINEN Intent zurück - den besten Match! Keine mehreren Intents.' : ''}`;
  ...
  try {
    ...
    const result = await aiWorkerPool.processRequest({
      ...
      systemPrompt:
        'Du bist ein präziser Intent-Klassifikator. Antworte NUR mit validem JSON Array.',
    ...
    let parsedIntents: Array<{
      agent: string;
      confidence?: number;
      params?: Record<string, unknown>;
    }> = [];
    let requestType: 'conversation' | 'document_query' | 'content_creation' | 'text_edit' =
      'content_creation';
    let subIntent: 'summarize' | 'translate' | 'compare' | 'explain' | 'brainstorm' | 'general' =
      'general';
...
export function getAvailableAgents(): Record<string, { description: string; keywords: string[] }> {
  return Object.entries(AGENT_MAPPINGS).reduce(
    (acc, [name, mapping]) => {
      acc[name] = {
