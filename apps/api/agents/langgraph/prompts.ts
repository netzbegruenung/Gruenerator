 ...
 * Generates optimized search queries for German political documents
 */
export function buildPlannerPromptGrundsatz(): PromptConfig {
  const system = [
 ...
 * Generates optimized search queries for any document collection
 */
export function buildPlannerPromptGeneral(): PromptConfig {
  const system = [
...
function buildDraftPrompt(collectionName: string, isPolitical: boolean): PromptConfig {
  ...
  const rules = [
    ...
    '- Nutze MÖGLICHST VIELE der verfügbaren Quellen. Wenn eine Quelle relevante Informationen enthält, beziehe sie ein.',
...
function buildConcisePrompt(collectionName: string, isPolitical: boolean): PromptConfig {
  ...
  const rules = [
    ...
    '## ANTWORT-STRUKTUR:',
    '- Maximal 3-4 Absätze',
    '- Beantworte die Frage VOLLSTÄNDIG: keine Ein-Satz- oder Ein-Absatz-Antworten, wenn die Quellen mehr hergeben',
    '- Strukturiere nach INHALTLICHEN Themen, nicht nach Dokumenten',
    ...
    '- Nutze möglichst viele relevante Quellen — nicht nur die offensichtlichsten.',
...
export function buildFastModePrompt(collectionName: string = 'Dokumente'): PromptConfig {
  const rules = [
    ...
    '- Antworte in normaler Länge (mehrere Absätze), nicht nur in Stichpunkten oder einem Kurzabsatz',
