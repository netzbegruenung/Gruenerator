    ...
    const response = await aiWorkerPool.processRequest(
      {
        ...
        options: {
          model: LANE.model,
          // Kein `max_tokens`: der Plan hat keine vorhersagbare Länge mehr —
          // eine Konsistenzprüfung listet jede nachrechenbare Angabe des
          // Materials, und ein abgeschnittener Plan ist ein STILLER Verlust.
          // Das JSON endet einfach, und die weggefallenen Zeilen sehen aus wie
          // Zeilen, die stimmen. Provider und Kontextfenster sind der Backstop
          // (dieselbe Regel wie auf allen Antwortpfaden, PR #2002).
          temperature: 0,
