...
export async function searchArgumentsFromNotebooks(
  ...
  const searchPromises = collections.map(async (collection) => {
    try {
      ...
      const result = await qdrant.hybridSearchDocuments(topicEmbedding, topic, {
        collection,
  ...
  const uniqueArguments = Array.from(new Map(allArguments.map((a) => [a.text, a])).values());

  return uniqueArguments.sort((a, b) => b.relevance - a.relevance).slice(0, limit);
}
