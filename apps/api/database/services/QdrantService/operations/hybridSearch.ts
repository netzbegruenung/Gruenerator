    ...
    try {
        ...
        return {
            ...
            metadata: {
                ...
                textMatchTypes: Array.from(new Set(textResults.map(r => r.matchType).filter(Boolean))) as string[]
            }
        };
...
export async function performTextSearch(
    ...
    try {
        ...
        const seen = new Map<string | number, { point: { id: string | number; payload: Record<string, unknown> }; variant: string; matchType: string }>();
        ...
        if (mergedPoints.length === 0) {
            ...
            if (tokens.length > 1) {
                ...
                const tokenSeen = new Map<string | number, { point: { id: string | number; payload: Record<string, unknown> }; variant: string; matchType: string }>();
 ...
 */
export function calculateTextSearchScore(searchTerm: string, text: string | undefined, position: number): number {
