...
export function createQdrantClient(config: QdrantClientConfig): QdrantClient {
    ...
    if (url.startsWith('https://')) {
        ...
        const basePath = parsedUrl.pathname && parsedUrl.pathname !== '/' ? parsedUrl.pathname : undefined;
...
export async function testConnectionWithRetry(
    ...
    throw new Error(`Qdrant connection failed after ${maxRetries} attempts: ${lastError?.message || 'Unknown error'}`);
}
