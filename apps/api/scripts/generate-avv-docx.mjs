...
function inlineRuns(text) {
  ...
  while ((m = linkRe.exec(text)) !== null) {
    ...
    runs.push(
      new ExternalHyperlink({ link: m[2], children: [new TextRun({ text: m[1], style: 'Hyperlink' })] }),
    );
...
function title(text) {
  return new Paragraph({ children: [new TextRun({ text, bold: true, size: 36 })], spacing: { after: 240 } });
}
...
const content = [
  ...
  { li: '**KI-Textgenerierung:** Erstellung von Pressemitteilungen, Social-Media-Beiträgen, Reden und weiteren Texten; das Modell ist pro Anfrage wählbar (Mistral AI/FR, eigene KI-Modelle der netzbegrünung e.V./EU, Seeweb/Regolo AI/IT; ergänzend IONOS für bestimmte Generatorfunktionen)' },
  ...
  { li: '**Audio- und Videotranskription:** Umwandlung von Sprach- und Videoaufnahmen in Text (Reel-Grünerator) über Seeweb/Regolo AI (Zero Data Retention), ersatzweise Mistral Voxtral' },
  ...
  { p: '**§ 3 Konkretisierung der Einzelmaßnahmen:**' },
  {
    table: {
      ...
      rows: [
        ['1', 'Verschlüsselung', 'End-to-End-Transportverschlüsselung (TLS/HTTPS); Verschlüsselung ruhender Daten in Datenbanken; SSH-Key-Only-Zugriff auf Server.'],
        ['2', 'Vertraulichkeit / Integrität', 'Härtung der Server (Hetzner/netzbegrünung); Firewalling; Patch-Management; Datentrennung durch Mandantenfähigkeit (Keycloak).'],
        ['3', 'Wiederherstellbarkeit', 'Tägliche Backups (verschlüsselt); geo-redundante Speicherung relevanter Daten.'],
        ['4', 'Identifizierung / Autorisierung', 'Starke Passwörter; 2FA für Admin-Zugänge; Zugriffsbeschränkung auf Moritz Wächter.'],
        ['5', 'Schutz bei Übermittlung', 'TLS (mind. 1.2/1.3); HSTS; API-Kommunikation mit Subunternehmern ausschließlich verschlüsselt.'],
        ['6', 'Physische Sicherheit', 'Nutzung ISO-27001-zertifizierter Rechenzentren der Dienstleister (Hetzner, IONOS).'],
        ['7', 'Datenminimierung', 'Zero-Retention bei Audio-/Video-Transkription (Seeweb/Regolo AI); Kurzzeit-Logs (max. 30 Tage) bei KI-Providern; lokale Browser-Speicherung für Entwürfe bevorzugt.'],
        ['8', 'Auftragskontrolle', 'Abschluss von AV-Verträgen (DPA) mit allen Subunternehmern; Deaktivierung von KI-Training (Opt-out).'],
      ],
    },
  },
  { spacer: true },
  ...
  { p: 'Nach § 8 Abs. 1 sind die bereits hinzugezogenen Subunternehmen zu bezeichnen; der Verantwortliche erklärt sich mit deren Beauftragung einverstanden.' },
  {
    table: {
      ...
      rows: [
        ...
        ['IONOS SE', 'Elgendorfer Str. 57, 56410 Montabaur, DE', 'Bestehend', 'Ergänzende KI-Textgenerierung (bestimmte Generatorfunktionen)', 'Verarbeitung in DE (EU); ISO-27001. Kein Training.'],
        ...
        ['Black Forest Labs ([Vertragsentität eintragen])', 'HQ Freiburg im Breisgau (DE); Vertragsentität noch zu bestätigen', 'Bestehend', 'Bildgenerierung (FLUX)', 'Verarbeitung in der EU (EU-API api.eu.bfl.ai bzw. FLUX bei IONOS/Regolo). Kein Training.'],
      ],
    },
  },
];
...
async function main() {
  ...
  console.log(`✓ Leistungsvereinbarung_und_AVV_Gruenerator.docx (${(buffer.length / 1024).toFixed(1)} KB) -> ${target}`);
}
