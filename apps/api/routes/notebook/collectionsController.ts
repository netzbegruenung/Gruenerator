...
async function resolveWolkeLinksToDocuments(
    ...
    try {
        ...
        `, [userId, wolkeShareLinkIds]) as unknown as DocumentRecord[];
...
router.get('/', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const collections = await notebookHelper.getUserNotebookCollections(userId) as NotebookCollectionFromQdrant[];

        const transformedData: TransformedCollection[] = await Promise.all(
            collections.map(async (collection) => {
                const documentIds = (collection.notebook_collection_documents || [])
