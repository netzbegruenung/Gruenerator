...
interface DreizeilenRequestBody {
  balkenGruppe_offset_x?: string;
  balkenGruppe_offset_y?: string;
  fontSize?: string;
  ...
  balkenOffset_0?: string;
  balkenOffset_1?: string;
  balkenOffset_2?: string;
  line1?: string;
  line2?: string;
  line3?: string;
  sunflower_offset_x?: string;
  sunflower_offset_y?: string;
  sunflowerPosition?: string;
  credit?: string;
}
...
async function processText(textData: TextData): Promise<TextLine[]> {
  ...
  const processedTextData: TextLine[] = [line1, line2, line3].map((line) => ({
    text: line || '',
  }));
...
async function addTextToImage(
  uploadedImageBuffer: Buffer | null,
  processedText: TextLine[],
  validatedParams: DreizeilenParams
): Promise<Buffer> {
  try {
    ...
    const canvas: Canvas = createCanvas(params.OUTPUT_WIDTH, params.OUTPUT_HEIGHT);
    const ctx: CanvasRenderingContext2D = canvas.getContext('2d');

    const { width: canvasWidth, height: canvasHeight } = canvas;

    if (hasBackgroundImage && img) {
    ...
    const balkenPositions: BalkenPosition[] = activeTextLines.map((line, index) => {
