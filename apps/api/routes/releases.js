...
const UPDATER_CONFIG = {
  ...
  platforms: {
    'linux-x86_64': {
      signature: '', // Paste signature from .sig file after building
...
// This endpoint serves the update manifest in the format required by @tauri-apps/plugin-updater
router.get('/updater/latest.json', (req, res) => {
  res.json(UPDATER_CONFIG);
});

// GET /api/releases/download/:platform - Serve release file for platform
// Platforms: linux-x86_64, darwin-x86_64, darwin-aarch64, windows-x86_64
...
router.get('/download/:platform/signature', (req, res) => {
  const { platform } = req.params;
  const version = UPDATER_CONFIG.version;
  ...
  if (!fs.existsSync(filePath)) {
    // Return signature from config if file doesn't exist
