...
// All optional fields use `.nullish()` so they accept both `undefined` and
// `null`. The frontend follows the project's no-undefined convention and
// sends `null` for unset values; plain `.optional()` would 400 every request.
// Mirrors packages/contracts/src/schemas/chatGraph.ts — keep in sync.
...
router.post(
  ...
  async (req: TypedRequest<ChatStreamBody>, res) => {
...
router.post(
  '/resume',
  validateBody(chatResumeSchema),
  async (req: TypedRequest<ChatResumeBody>, res) => {
    const sse = createSSEStream(res);
