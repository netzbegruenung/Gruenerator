...
router.get('/documentation', (req: Request, res: Response): void => {
  res.json({
    ...
    process: {
      description:
        'The offboarding process fetches users from the Grüne API, searches for them in the Grünerator database, attempts to delete them (or anonymize if deletion fails), and reports the results back to the API.',
      retry_mechanism:
        'Failed API updates are saved to /var/tmp/gruenerator-offboarding-retry.json and retried on next run',
