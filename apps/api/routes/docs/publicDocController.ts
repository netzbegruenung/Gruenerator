  ...
  try {
    ...
    const result = await db.query(
      `SELECT id, title, share_permission, document_subtype
       ...
       WHERE id = $1 AND is_public = true AND is_deleted = false AND document_subtype = 'docs'`,
      [id]
    ) as unknown as { id: string; title: string; share_permission: string; document_subtype: string }[];
