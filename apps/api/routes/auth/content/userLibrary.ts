...
import type {
  AuthRequest,
  SaveToLibraryBody,
  SavedTextMetadataBody,
  SavedTextContentBody,
  BulkDeleteBody,
  SearchSavedTextsBody,
} from '../types.js';
...
const VALID_TYPES = [
  ...
  'actionIdeas',
  'reelScript',
  'gruene_jugend_instagram',
  'gruene_jugend_twitter',
  'gruene_jugend_tiktok',
  'gruene_jugend_messenger',
  'gruene_jugend_reelScript',
  'gruene_jugend_actionIdeas',
  'kleine_anfrage',
  'grosse_anfrage',
  'rede',
  'wahlprogramm',
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const {
        ...
        title: manualTitle,
        type = 'universal',
      } = req.body as SaveToLibraryBody & { type?: string };
      ...
      log.debug(
        `[User Content /save-to-library POST] Successfully saved text ${data.id} for user ${userId}`
      );
      ...
      setImmediate(async () => {
        try {
          ...
          if (await qdrant.isAvailable()) {
            ...
            log.debug(
              `[Vectorization] Vectorized ${chunks.length} chunks for document ${documentId}`
            );
          }
        } catch (vectorError) {
...
router.post(
  ...
  async (req: AuthRequest<{ id: string }>, res: Response): Promise<void> => {
    try {
      const userId = req.user!.id;
      const { id } = req.params;
      const { title, document_type } = req.body as SavedTextMetadataBody & {
