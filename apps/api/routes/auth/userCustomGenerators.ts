...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      const userId = req.user!.id;
      const { name, slug, title, description, form_schema, prompt, contact_email } =
        req.body as CustomGeneratorCreateBody;
...
router.delete(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      log.debug(
        `[User Custom Generators] Generator "${existingGenerator.name}" (${id}) deleted by user ${userId}`
      );
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      log.debug(
        `[User Custom Generators] Generator "${generator.name}" (${generatorId}) saved by user ${userId}`
      );
...
router.delete(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      log.debug(
        `[User Custom Generators] Saved generator ${generatorId} removed by user ${userId}`
      );
