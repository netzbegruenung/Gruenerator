...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      const authHeader = req.headers.authorization;
      const redirect = (req.body as Record<string, unknown> | undefined)?.redirect as string;
...
router.get('/mobile/session-bridge', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // TODO: Create Better Auth session (req.login from Passport.js was removed)
    // For now, redirect without a web session — the WebView will need to
    // authenticate via Bearer token instead.
    log.info('[SessionBridge] Bridge code consumed, redirecting', {
      userId: entry.userId,
