...
export async function fetchRecentImages(
  ...
  return rows.map((row) => ({
    ...
    // One signed URL regardless of whether the async variants pass has finished:
    // the endpoint hits the pre-generated 400px variant when it exists and
    // renders on demand when it does not, so there is nothing left to branch on.
    thumbnailUrl:
      buildThumbnailTileUrl('media', row.share_token, versionFromDate(row.created_at)) ?? undefined,
...
export async function fetchRecentCanvases(
  ...
  return (
    ...
    // The stored URL points at /api/share/<token>/download, which requires
    // auth — passing it through is why canvas tiles were blank even when a
    // thumbnail existed.
    thumbnailUrl: buildCanvasThumbnailUrl(row.id, row.thumbnail_url) ?? undefined,
