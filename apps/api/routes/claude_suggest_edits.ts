...
import type { EditChange, EditSuggestionResult, GenerationContext, JsonParsable } from '../types/routes.js';
...
function parseJsonSafe(raw: JsonParsable): EditSuggestionResult | null {
  if (typeof raw !== 'string') return raw as EditSuggestionResult;
...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  ...
  if (componentName && req.session?.id) {
    try {
      const { default: redisClient } = await import('../utils/redis/RedisClient.js');
      const contextKey = `edit_context:${req.session.id}:${componentName}`;

      const cached = await redisClient.get(contextKey);
      if (cached) {
        generationContext = JSON.parse(cached) as GenerationContext;
  ...
  try {
    const { buildEditContextSummary } = await import('../utils/request/editContextBuilder.js');
