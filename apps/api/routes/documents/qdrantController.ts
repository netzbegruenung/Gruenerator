...
router.get(
  ...
  async (req: DocumentRequest, res: Response): Promise<void> => {
    try {
      ...
      log.debug(
        `[GET /:documentId/chunk-context] Fetching context for doc: ${documentId}, chunk: ${chunkIdx}, window: ${windowSize}, collection: ${collection}`
      );
      ...
      if (collectionType === 'user' && !isValidUUID(documentId)) {
        log.debug(
          `[GET /:documentId/chunk-context] Document ID '${documentId}' is not a UUID, auto-detecting system collection`
        );
      ...
      if (collectionType === 'user') {
        ...
        result = await documentSearchService.getChunkWithContext(userId, documentId, chunkIdx, {
          window: windowSize,
        });
      } else {
        // System collections (grundsatz, gruene_de, bundestag, etc.)
        result = await documentSearchService.getSystemChunkWithContext(
          collectionType,
          documentId,
          chunkIdx,
          { window: windowSize }
      ...
      log.debug(
        `[GET /:documentId/chunk-context] Retrieved ${result.contextChunks?.length || 0} context chunks`
      );
...
router.get('/stats', async (req: DocumentRequest, res: Response): Promise<void> => {
  try {
    ...
    log.debug(
      `[GET /stats] User has ${stats.totalVectors} vectors across ${stats.uniqueDocuments} documents`
    );
