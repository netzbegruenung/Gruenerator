 ...
 */
router.post('/prompt', requireAuth, upload.single('image'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!limitStatus.canGenerate) {
      log.debug(`[Image Edit] Request rejected: User ${userId} has reached daily limit (${limitStatus.count}/${limitStatus.limit})`);
    ...
    log.debug(`[Image Edit] Processing request with instruction: "${userText?.substring(0, 100)}..." (User: ${userId}, Usage: ${limitStatus.count + 1}/${limitStatus.limit}, Precision: ${isPrecision})`);
    ...
    log.debug(`[Image Edit] Processing image: ${req.file.originalname}, size: ${Math.round(req.file.size / 1024)}KB, type: ${req.file.mimetype}`);
    ...
    log.debug(`[Image Edit] Image generation completed successfully, output size: ${Math.round(stored.size / 1024)}KB`);
 ...
 */
router.post('/generate', requireAuth, upload.single('image'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    ...
    const limitStatus = await imageCounter.checkLimit(userId);
    if (!limitStatus.canGenerate) {
      log.debug(`[Image Edit Generate] Request rejected: User ${userId} has reached daily limit (${limitStatus.count}/${limitStatus.limit})`);
      return res.status(429).json({
        success: false,
        error: 'Daily image generation limit reached',
        data: limitStatus,
    ...
    const userText = body.text || body.instruction || '';
    const isPrecision = body.precision === 'true' || body.precision === true;
    log.debug(`[Image Edit Generate] Processing request with instruction: "${userText?.substring(0, 100)}..." (User: ${userId}, Usage: ${limitStatus.count + 1}/${limitStatus.limit}, Precision: ${isPrecision})`);
    ...
    if (req.file) {
      ...
      generationResult = await flux.generateFromPrompt(
        prompt,
        { output_format: 'jpeg', safety_tolerance: 2 }
      ) as FluxGenerationResult;
    }
    ...
    log.debug(`[Image Edit Generate] Image generation completed successfully, output size: ${Math.round(stored.size / 1024)}KB`);
