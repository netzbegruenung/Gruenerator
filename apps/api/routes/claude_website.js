...
router.post('/', async (req, res) => {
  const { description, email, usePrivacyMode, useProMode } = req.body;
...
${description}`;
    ...
    // Fix newlines inside JSON string values (common issue with Mistral)
    // This regex finds strings and replaces internal newlines with \n
    ...
    try {
      ...
      log.error('[claude_website] JSON parse error:', parseError.message);
    ...
    // Basic validation
    const requiredFields = ['hero', 'about', 'hero_image', 'themes', 'actions', 'contact'];
    ...
    // Ensure exactly 3 themes by slicing or padding
    ...
    // Ensure exactly 3 actions by slicing or using defaults
