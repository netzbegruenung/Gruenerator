...
export interface ContentPath {
  ...
  wolkeShare?: { shareKey: string; recursive?: boolean; excludeNames?: string[] }; // Optional: pull documents from a public Nextcloud "Wolke" share via WebDAV instead of HTML/WP discovery. The share link is a credential and never lives in this public repo: `shareKey` names its entry in <INTERN_CONTENT_DIR>/wolke-shares.json (see utils/wolkeShareSecrets.ts). Files are etag-deduped, so an unchanged file is skipped before download+OCR. `excludeNames` adds extra case-insensitive substrings to `isExcludedWolkeFileName`'s defaults (entwurf/vorlage/anschreiben/formular/intern), matched on the file name only. Reusable by any source; see services/scrapers/utils/wolkeShareHandler.ts.
