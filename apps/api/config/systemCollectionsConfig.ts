 ...
 */
export function buildSubcategoryFilter(subcategoryFilters: SubcategoryFilters | null | undefined): QdrantFilter | undefined {
    if (!subcategoryFilters || Object.keys(subcategoryFilters).length === 0) {
        return undefined;
    }

    const must: Array<{ key: string; match?: { value?: string; any?: string[] }; range?: { gte?: string; lte?: string } }> = [];
    const filterKeys: (keyof SubcategoryFilters)[] = ['primary_category', 'content_type', 'subcategories', 'country', 'region'];
