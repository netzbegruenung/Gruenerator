...
class QueryIntentService {
   ...
   *
   * Returns an empty filter. Content-type and language preferences are NOT
   * added as Qdrant `should` clauses because in Qdrant, when `should` coexists
   * with `must`, at least one `should` must match — turning soft preferences
   * into hard filters. This caused 0 results on collections whose documents
   * have different content_type values (e.g. "beschluss" in Hamburg).
   * Quality-based re-ranking in searchWithQuality() handles result boosting.
   */
  generateSearchFilters(_intent: IntentDetectionResult): QdrantFilter {
    return {};
  }
