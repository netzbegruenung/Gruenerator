/**
 * The agent's three retrieval tools.
 *
...
export interface ToolContext {
  ...
  /** Every URL the run saw, in first-seen order — the report's source list. */
 ...
 */
function localeHint(locale: ResearchLocale): { greenpt: string; queryNote: string } {
  return locale === 'de-AT'
    ? { greenpt: 'de-AT', queryNote: ' (Österreich)' }
    : { greenpt: 'de-DE', queryNote: '' };
}

function remember(ctx: ToolContext, url: string, title: string): void {
  if (!url) return;
  if (!ctx.sources.has(url)) ctx.sources.set(url, { url, title: title || url });
}

/** Compact, numbered, one line per hit — cheap in tokens, easy for a model to cite. */
function formatHits(hits: { url: string; title: string; snippet: string }[]): string {
...
function budgetSpent(ctx: ToolContext): string | null {
...
export function createResearchTools(ctx: ToolContext) {
  ...
  return [webSuche, tiefenSuche, seiteLesen];
}
