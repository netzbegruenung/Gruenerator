...
class EnrichedPersonSearchService {
    constructor() {
     ...
     * @returns {Promise<Object>} Enriched search result or null if not a person query
     */
    ...
    async _searchBundestagContent(personName, limit = 15) {
        try {
            ...
            const searchResults = await this.qdrant.client.search('bundestag_content', {
                vector: embedding,
                limit: limit * 2,
                with_payload: true,
                score_threshold: 0.3
            ...
            const textResults = await this.qdrant.client.scroll('bundestag_content', {
                filter: {
                    must: [
...
function getEnrichedPersonSearchService() {
    if (!serviceInstance) {
...
export { EnrichedPersonSearchService, getEnrichedPersonSearchService };
