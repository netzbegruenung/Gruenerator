...
export function getQualitySettings(referenceDimension: number, isLargeFile = false): QualitySettings {
  if (isLargeFile) {
    return { crf: QUALITY_CONFIG.CRF.LARGE, qp: QUALITY_CONFIG.QP.LARGE, preset: QUALITY_CONFIG.PRESET.LARGE_FILE, audioBitrate: QUALITY_CONFIG.AUDIO['1080p'] };
  }

  if (referenceDimension >= 2160) {
    return { crf: QUALITY_CONFIG.CRF['4K'], qp: QUALITY_CONFIG.QP['4K'], preset: QUALITY_CONFIG.PRESET.STANDARD, audioBitrate: QUALITY_CONFIG.AUDIO['4K'] };
  } else if (referenceDimension >= 1440) {
    return { crf: QUALITY_CONFIG.CRF['1440p'], qp: QUALITY_CONFIG.QP['1440p'], preset: QUALITY_CONFIG.PRESET.STANDARD, audioBitrate: QUALITY_CONFIG.AUDIO['1440p'] };
  } else if (referenceDimension >= 1080) {
    return { crf: QUALITY_CONFIG.CRF['1080p'], qp: QUALITY_CONFIG.QP['1080p'], preset: QUALITY_CONFIG.PRESET.STANDARD, audioBitrate: QUALITY_CONFIG.AUDIO['1080p'] };
  } else if (referenceDimension >= 720) {
    return { crf: QUALITY_CONFIG.CRF['720p'], qp: QUALITY_CONFIG.QP['720p'], preset: QUALITY_CONFIG.PRESET.STANDARD, audioBitrate: QUALITY_CONFIG.AUDIO['720p'] };
  }
  return { crf: QUALITY_CONFIG.CRF.SD, qp: QUALITY_CONFIG.QP.SD, preset: QUALITY_CONFIG.PRESET.STANDARD, audioBitrate: QUALITY_CONFIG.AUDIO.SD };
}
...
export async function detectVaapi(): Promise<boolean> {
  ...
  try {
    ...
    hwaccelAvailable = false;
    const output = err.stderr?.toString()?.trim() || err.stdout?.toString()?.trim() || err.message || 'unknown error';
...
export function getSubtitleFilterChain(assFilePath: string | null, fontDir: string | null, scaleFilter: string | null = null): string {
