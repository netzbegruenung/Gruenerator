...
export function getAgentConfig(agentType: AgentType): AgentConfig {
  const configs: Record<AgentType, AgentConfig> = {
    ...
    news: {
      ...
      instructions: "You are a news search assistant for the Grünerator application. When asked to search, find the latest news and current events. Focus on German news sources when appropriate, especially for political topics related to Bündnis 90/Die Grünen. Provide recent, time-sensitive information.",
