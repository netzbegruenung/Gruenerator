...
import { simpleHash } from '../utils/hashUtils.js';
...
class AISearchAgent {
    ...
    async searchDatabase(
        ...
        try {
            ...
            log.debug(`Extracted intent: query=${searchIntent.query}, contentType=${searchIntent.contentType}, limit=${searchIntent.limit}`);
            ...
            log.debug(`Autonomous search completed: ${searchResult.elementIds?.length || 0} results, confidence: ${searchResult.confidence}`);
            return searchResult;
...
WICHTIG: Antworte NUR mit reinem JSON. KEINE Markdown-Code-Blöcke, KEINE Backticks, KEINE Erklärungen. Nur das reine JSON-Objekt.`;
     ...
     */
    private buildEnhancementUserContent(query: string, contentType: string, _options: EnhanceQueryOptions): string {
...
Antworte NUR mit diesem JSON (keine Markdown-Blöcke):
{
  ...
  "confidence": 0.85
    ...
    private parseAIResponse(aiResult: AIWorkerResult, originalQuery: string): EnhancementResult {
        try {
            ...
            return {
                ...
                semanticContext: jsonResponse.semanticContext || { mainTopics: [], relatedTerms: [], politicalContext: '' },
     ...
     */
    private getFallbackEnhancement(query: string | null | undefined, errorMessage: string | null = null): EnhancementResult {
        ...
        return {
            success: true,
            originalQuery: query || '',
            enhancedQueries: [query, ...basicExpansions].filter((q): q is string => Boolean(q)).slice(0, 3),
            semanticContext: {
                mainTopics: [],
     ...
     */
    extractSearchIntent(naturalLanguageQuery: string, passedContentType: string | null = null): SearchIntent {
...
Nach der Suche erkläre deine Keyword-Wahl und begründe die Relevanz der Ergebnisse.`;
            ...
            try {
                ...
                switch (toolCall.name) {
                    case 'search_database_examples':
                        toolResult = await this.handleSearchDatabaseExamples(toolCall.input as unknown as SearchExamplesParams);
                        break;
                    case 'execute_database_query':
                        toolResult = await this.handleExecuteDatabaseQuery(toolCall.input as unknown as DatabaseQueryParams);
                        break;
                    case 'analyze_and_rank_results':
                        toolResult = await this.handleAnalyzeAndRankResults(toolCall.input as unknown as AnalyzeAndRankParams);
                        break;
                    default:
    ...
    private async handleSearchDatabaseExamples(params: SearchExamplesParams): Promise<SearchToolResult> {
        ...
        log.debug(`🔍 AI SEARCH PARAMETERS: query=${params.query}, contentType=${params.content_type}, limit=${enforcedLimit}, threshold=${params.threshold || 0.15}`);
        ...
        const resultsWithMetadata = ((result.results || []) as unknown as SearchResultItem[]).map((item) => ({
            ...item,
     ...
     */
    private async handleExecuteDatabaseQuery(params: DatabaseQueryParams): Promise<DatabaseQueryResult> {
     ...
     */
    private aggregateMultiSearchResults(toolResults: ToolExecutionResult[], targetLimit: number): SearchResultItem[] {
        ...
        for (const toolResult of toolResults) {
            ...
            if (result?.success && result.results && Array.isArray(result.results)) {
                ...
                log.debug(`❌ Skipping tool result: success=${result?.success}, hasResults=${!!result?.results}`);
            }
        }

        log.debug(`🔄 AGGREGATING MULTI-SEARCH RESULTS: totalSearches=${toolResults.length}, totalResultsFound=${allResults.length}, targetLimit=${targetLimit}`);
        ...
        log.debug(`📈 MULTI-SEARCH AGGREGATION COMPLETE: beforeDedup=${allResults.length}, afterDedup=${deduplicatedResults.length}, finalSelected=${selectedResults.length}`);
    ...
    private buildMultiSearchReasoning(
        ...
        const totalResults = toolResults.reduce((sum, tr) => sum + ((tr.result as SearchToolResult)?.count || 0), 0);
     ...
     */
    private calculateConfidence(toolResults: ToolExecutionResult[], searchIntent: SearchIntent): number {
