...
export interface PdfFormField {
  ...
  /** Whether the field has a widget that can actually SHOW a value: a non-zero
   *  rectangle that is not flagged hidden.
   *
   *  Not academic. Forms generated by MACH formsolutions (widespread in German
   *  municipal administration) carry a full set of named fields whose widgets
   *  are all `/Rect [0 0 0 0]`. Writing to them succeeds and reads back
   *  correctly — and produces a completely blank document. Filling those and
...
export interface FillResult {
  ...
  /** Post-condition check on the FINISHED document: how many written values are
   *  actually readable on the page. Absent when the check could not run. */
  verification?: { visible: number; missing: string[] };
}
...
/** Annotation flag bit 2 = Hidden (PDF 32000-1, table 165). */
const ANNOT_HIDDEN = 1 << 1;

/**
 * Can this field's value ever be seen? True when at least one widget has a
 * non-degenerate rectangle and is not hidden.
 */
async function isRenderable(field: { getName(): string }): Promise<boolean> {
  const { PDFName } = await import('pdf-lib');
  const widgets = (
    field as unknown as {
      acroField: { getWidgets(): Array<{ dict: { get(k: unknown): unknown } }> };
    }
  ).acroField.getWidgets();

  return widgets.some((w) => {
    const rect = String(w.dict.get(PDFName.of('Rect')) ?? '');
    const n = rect.replace(/[[\]]/g, '').trim().split(/\s+/).map(Number);
    if (n.length < 4 || n.some(Number.isNaN)) return false;
    const area = Math.abs((n[2] - n[0]) * (n[3] - n[1]));
    if (area === 0) return false;
    const flags = Number(w.dict.get(PDFName.of('F')) ?? 0);
    return (flags & ANNOT_HIDDEN) === 0;
  });
}
...
export async function readFormFields(bytes: Buffer): Promise<PdfFormField[]> {
  ...
  return await Promise.all(
    form.getFields().map(async (field): Promise<PdfFormField> => {
      const name = field.getName();
      const readOnly = field.isReadOnly();
      const renderable = await isRenderable(field);
      ...
      return { name, type: 'text', renderable: false, options: [], value: null, readOnly: true };
    })
  );
}
...
export async function fillFormFields(
  ...
  for (const [name, rawValue] of Object.entries(values)) {
    ...
    if (!(await isRenderable(field))) {
      skipped[name] = 'Feld hat keine sichtbare Position im Dokument';
      continue;
    }
  ...
  const outBytes = Buffer.from(saved);

  // Trust nothing: read the finished document back and check the values are
  // actually ON the page. A write can succeed at the data layer and still
  // render blank (degenerate widgets, broken appearance streams) — that class
  // of failure is invisible to the writer and would otherwise be reported as
  // success. Verification is best-effort: if it cannot run, the fill stands.
  const written = filled
    .map((name) => String(values[name] ?? ''))
    .filter((v) => v.trim().length > 0);
  const verified = await verifyValuesVisible(outBytes, written);

  log.info(
    `[pdfForm] Filled ${filled.length} field(s), skipped ${Object.keys(skipped).length}` +
      (verified === null ? '' : `, sichtbar: ${verified.visible}/${written.length}`)
  );
  return {
    bytes: outBytes,
    filled,
    skipped,
    ...(verified !== null && { verification: verified }),
  };
}

/**
 * Extract the finished PDF's text layer and report how many of the written
 * values appear in it. Returns null when the check itself could not run —
 * an unavailable verifier must never turn a good fill into a failure.
 */
async function verifyValuesVisible(
  bytes: Buffer,
  written: string[]
): Promise<{ visible: number; missing: string[] } | null> {
  if (written.length === 0) return { visible: 0, missing: [] };
  try {
    // pdfjs directly, not OCRService: the text layer is all this needs, and the
    // OCR service drags in Mistral/docling/DB wiring that can fail for reasons
    // having nothing to do with the fill.
    ...
    for (let p = 1; p <= doc.numPages; p++) {
      ...
      haystack += content.items
        .map((i) =>
          typeof (i as { str?: string }).str === 'string' ? (i as { str: string }).str : ''
        )
        .join('');
    }
    const norm = (s: string) => s.replace(/\s+/g, '').trim();
    const flat = norm(haystack);
    // No text layer at all: cannot judge, so do not pretend to.
    if (!flat) return null;

    const missing = written.filter((v) => !flat.includes(norm(v)));
    return { visible: written.length - missing.length, missing };
  } catch (error) {
    log.warn(`[pdfForm] Verification skipped: ${error instanceof Error ? error.message : error}`);
    return null;
  }
}
