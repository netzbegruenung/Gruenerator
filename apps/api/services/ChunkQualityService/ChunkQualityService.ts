...
class ChunkQualityService {
   ...
   */
  calculateQualityScore(text: string, metadata: ChunkMetadata = {}): number {
   ...
   * @returns Readability score in [0,1]
   */
   ...
   * @param text - Text to analyze
   * @returns Completeness score in [0,1]
   */
  calculateCompleteness(text: string): number {
   ...
   * @param text - Text to analyze
   * @param metadata - Optional metadata (may include contentType)
   * @returns Structure score in [0,1]
   */
  calculateStructureScore(text: string, metadata: ChunkMetadata = {}): number {
   ...
   * @param text - Text to analyze
   * @returns Information density score in [0,1]
   */
  ...
  /**
   * Tokenize text into lowercase normalized words
   */
  ...
  /**
   * Bounded score calculation for deviation-based metrics
   */
  #boundedScore(value: number, min: number, max: number): number {
  ...
  /**
   * Normalize value to [0,1] within min/max range
   */
  ...
  /**
   * German stopwords set for density calculation
   */
...
export { ChunkQualityService, chunkQualityService };
