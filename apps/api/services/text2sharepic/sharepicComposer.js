...
class SharepicComposer {
   ...
   */
  async generateFromPlan(layoutPlan, options = {}) {
    const startTime = Date.now();
  ...
  async renderLayoutPlan(plan) {
    ...
    for (const zoneConfig of plan.zones) {
      ...
      const zoneDef = templateZones.find(z => z.name === zoneConfig.zoneName);
      if (!zoneDef) {
   ...
   */
  mergeContentIntoParams(zoneConfig, content) {
    ...
    if (contentFields && content) {
      for (const field of contentFields) {
        if (content[field] && !params.text && !params.quote) {
          if (zoneConfig.component === 'text-quote') {
            params.text = content[field];
   ...
   */
  generateCacheKey(description, options) {
   ...
   */
  async generateVariants(description, count = 4, options = {}) {
   ...
   */
  async updateAndRender(layoutPlan, contentUpdates) {
    const updatedPlan = {
      ...layoutPlan,
   ...
   *
   * @param {string} description - User's description of the desired sharepic
   * @param {object} req - Express request object (required for AI worker pool access)
   * @param {object} options - Additional options
   * @returns {Promise<object>} Generated sharepic result
   */
  async generateFromAI(description, req, options = {}) {
    const startTime = Date.now();
    ...
    try {
      ...
      const backgroundZone = zones.find(z => z.zoneName === 'background' && z.component === 'background-image');
   ...
   */
  getTemplatesForContent(contentType) {
   ...
   *
   * @param {object} currentLayoutPlan - The current layout plan to edit
   * @param {string} editRequest - User's description of desired changes
   * @param {object} req - Express request object (required for AI worker pool access)
   * @returns {Promise<object>} Updated sharepic result
   */
  async editFromAI(currentLayoutPlan, editRequest, req) {
    const startTime = Date.now();
   ...
   */
  previewLayoutPlan(description, options = {}) {
 ...
 */
function createSharepicComposer(options = {}) {
 ...
 */
async function quickGenerate(description, options = {}) {
  const composer = new SharepicComposer(options);
