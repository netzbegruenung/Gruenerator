  ...
  const generateSharepic = useCallback(
    async (
      ...
      try {
        ...
        switch (sharepicType) {
          case 'default':
            return await generateDefaultSharepics(
              thema,
              details,
              customPrompt,
              attachments,
          ...
          case 'quote':
            return await generateUnifiedSharepic(
              'zitat',
              thema,
              details,
              uploadedImage,
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Fehler bei der Sharepic-Generierung';
    ...
    async (
      ...
      if (uploadedImage && (uploadedImage instanceof File || uploadedImage instanceof Blob)) {
        try {
          ...
          console.warn(
            '[useSharepicGeneration] Failed to convert original image to base64:',
            error
          );
        }
      }
