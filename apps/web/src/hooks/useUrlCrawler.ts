...
export const useUrlCrawler = (initialUrls: string[] = []) => {
  ...
  const crawlUrl = useCallback(
    async (url: string, usePrivacyMode = false): Promise<CrawledAttachment | null> => {
      ...
      if (inFlightUrlsRef.current.has(url)) {
        console.log(
          `[useUrlCrawler] URL already being processed, skipping duplicate start: ${url}`
        );
        return null;
      }
      ...
      try {
        ...
        if (response.success) {
          ...
          setCrawledUrls((prev) => {
            ...
            if (exists) {
              return prev.map((item) =>
                item.url === response.attachment.url ? response.attachment : item
              );
            }
          ...
          console.log(
            `[useUrlCrawler] Successfully crawled: ${url} (${response.attachment.metadata?.wordCount || 0} words)`
          );
   ...
   */
  const detectAndCrawlUrls = useCallback(
    async (text: string, usePrivacyMode = false): Promise<CrawledAttachment[]> => {
      ...
      const successfulCrawls = results
        .filter(
          (r): r is PromiseFulfilledResult<CrawledAttachment | null> =>
            r.status === 'fulfilled' && r.value !== null
        )
        .map((r) => r.value as CrawledAttachment);

      console.log(
        `[useUrlCrawler] Successfully crawled ${successfulCrawls.length} of ${newUrls.length} URLs`
      );
  ...
  const getErrors = useCallback(
    (url: string | null = null): string | null | Record<string, string | null> => {
      if (url) {
