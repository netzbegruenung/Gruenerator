...
export async function fetchAdminVorlagen(status = 'pending_review') {
  ...
  if (result.status !== 200) {
    throw new ApiError(result.status, `Failed to fetch admin vorlagen (HTTP ${result.status})`);
  }
...
export async function fetchVorlagenStats() {
  ...
  if (result.status !== 200) {
    throw new ApiError(result.status, `Failed to fetch vorlagen stats (HTTP ${result.status})`);
  }
  return result.body.data;
}
...
export async function approveVorlage(id: string, message?: string): Promise<void> {
  ...
  if (result.status !== 200) {
    throw new ApiError(result.status, `Failed to approve vorlage (HTTP ${result.status})`);
  }
}
...
export async function rejectVorlage(id: string, reason?: string): Promise<void> {
  ...
  if (result.status !== 200) {
    throw new ApiError(result.status, `Failed to reject vorlage (HTTP ${result.status})`);
  }
}
