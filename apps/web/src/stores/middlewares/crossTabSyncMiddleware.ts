...
interface ConfigWithCrossTabSync<T> {
  ...
  (set: (partial: Partial<T> | ((state: T) => Partial<T>), replace?: boolean) => void, get: () => T, api: StoreApi<T>): T;
}

type SetState<T> = (partial: Partial<T> | ((state: T) => Partial<T>), replace?: boolean) => void;
type GetState<T> = () => T;

export const crossTabSyncMiddleware = <T extends object>(
  config: ConfigWithCrossTabSync<T>
) => (
  set: SetState<T>,
  get: GetState<T>,
  api: StoreApi<T> & { destroy?: () => void }
): T => {
 ...
 */
export const createCrossTabSyncConfig = (storageKey: string, syncKeys: string[] | null = null): { crossTabSync: CrossTabSyncConfig } => ({
  crossTabSync: {
    storageKey,
    syncKeys
  }
