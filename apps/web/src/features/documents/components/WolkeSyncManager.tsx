...
export const WolkeSyncManager = ({
  wolkeShareLinks = [],
  onRefreshShareLinks,
  onSyncComplete,
}: WolkeSyncManagerProps): React.ReactElement => {
  ...
  const handleConfigureWolke = () => {
    void navigate('/profile/integrationen/wolke');
  };
  ...
  if (initialized && !shareLinksLoading && !hasShareLinks && !loading) {
    ...
    const message =
      'Richte zunächst deine Nextcloud-Verbindungen ein, um die Wolke-Synchronisation zu nutzen.';
    ...
    return (
      <div className="wolke-sync-manager wolke-not-configured">
                ...
                {(() => {
                  ...
                  return RefreshIcon
                    ? React.createElement(RefreshIcon, {
                        className: `pabtn__icon ${shareLinksLoading ? 'spinning' : ''}`,
                      })
                    : null;
                })()}
        ...
        <div className="wolke-configure-prompt">
          <div className="configure-message">
            <p>{message}</p>
            <button className="pabtn pabtn--primary pabtn--m" onClick={handleConfigureWolke}>
              {(() => {
                const CloudIcon = getIcon('actions', 'cloud');
                return CloudIcon
                  ? React.createElement(CloudIcon, { className: 'pabtn__icon' })
                  : null;
              })()}
    ...
    <div className="wolke-sync-manager">
      <div className="wolke-header-minimal">
        ...
        {!showBrowser && selectedFolder && (
          <button className="pabtn pabtn--ghost pabtn--s" onClick={handleBackToBrowser}>
...
const ShareLinksWithSync = ({
  shareLinks,
  syncStatuses,
  onSyncFolder,
  onSyncComplete,
  refreshSyncStatuses,
}: ShareLinksWithSyncProps): React.ReactElement => {
  ...
  const getSyncStatus = (shareLinkId: string): SyncStatus | null => {
    const syncStatus = syncStatuses.find(
      (status: SyncStatus) => status.share_link_id === shareLinkId
    );
    ...
    <div className="wolke-share-links-simple">
      {shareLinks.map((shareLink) => {
        ...
        const isCurrentlySyncing =
          syncingFolders.has(`${shareLink.id}-`) || syncStatus?.sync_status === 'syncing';
              ...
              <div className="share-link-header">
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      const result = (await setAutoSync(
                        shareLink.id,
                        '',
                        newCheckedState
                      )) as AutoSyncResult;
                      ...
                      setTimeout(async () => {
                        try {
                          ...
                          console.error(
                            '[ShareLinksWithSync] Failed to refresh sync statuses:',
                            refreshError
                          );
                        }
                      }, 100);
                      ...
                      if (result?.success && result?.autoSyncEnabled && newCheckedState) {
                        try {
                          ...
                          console.error(
                            '[WolkeSyncManager] Auto-sync initial sync failed:',
                            syncError
                          );
                        }
                      }
              ...
              {isSyncEnabled && (
                ...
                >
                  {(() => {
                    ...
                    return RefreshIcon
                      ? React.createElement(RefreshIcon, {
                          className: `pabtn__icon ${isCurrentlySyncing ? 'spinning' : ''}`,
                        })
                      : null;
                  })()}
                </button>
