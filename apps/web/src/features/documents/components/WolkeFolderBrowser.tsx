...
export const WolkeFolderBrowser = ({
  ...
  const { syncFolder, setAutoSync, getSyncStatus, isSyncing } = useWolkeSync();
  ...
  const handleToggleAutoSync = async (shareId: string, folderPath = '', enabled: boolean) => {
    try {
      ...
      if (
        result &&
        result.success &&
        (result as { autoSyncEnabled?: boolean }).autoSyncEnabled &&
        enabled
      ) {
        try {
          await syncFolder(shareId, folderPath);
    ...
    return (
      <div className="wolke-folder-browser">
        <div className="error-state">
          ...
          <button onClick={loadShareLinks} className="retry-button">
            Erneut versuchen
          </button>
        </div>
      </div>
