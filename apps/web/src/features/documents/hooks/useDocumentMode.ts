...
type DocumentMode = 'wolke' | 'manual' | 'unified';

/**
 * Custom hook for managing document mode (manual/wolke)
 */
export const useDocumentMode = () => {
  const [currentMode, setCurrentMode] = useState<DocumentMode>('wolke');
  ...
  return {
    currentMode: 'unified' as const, // Always return unified mode
