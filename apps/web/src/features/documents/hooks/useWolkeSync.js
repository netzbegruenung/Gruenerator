...
export const useWolkeSync = (useStore = true) => {
  ...
  const setAutoSync = useCallback(
    async (shareLinkId, folderPath = '', enabled) => {
      if (useStore) {
      ...
      try {
        ...
        if (response.data && response.data.success) {
          console.log(
            `[useWolkeSync] Auto-sync ${enabled ? 'enabled' : 'disabled'} for ${shareLinkId}`
          );
        ...
        setLocalError(
          `Fehler beim ${enabled ? 'Aktivieren' : 'Deaktivieren'} der Auto-Synchronisation`
        );
        throw error;
      }
    },
