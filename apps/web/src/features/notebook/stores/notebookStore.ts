...
const useNotebookStore = create<NotebookState>((set, get) => ({
  ...
  getEnhancedCollection: (collectionId) => {
    ...
    return {
      ...
      total_sources:
        (collection.documents || []).length + (collection.wolke_share_links || []).length,
      is_mixed_sources:
        (collection.documents || []).length > 0 && (collection.wolke_share_links || []).length > 0,
    };
  },
  ...
  getCollectionsBySourceType: (sourceType) => {
    ...
    return qaCollections.filter((collection) => {
      if (sourceType === 'documents') {
        return (
          (collection.documents || []).length > 0 && !(collection.wolke_share_links || []).length
        );
      } else if (sourceType === 'wolke') {
        return (
          (collection.wolke_share_links || []).length > 0 && !(collection.documents || []).length
        );
