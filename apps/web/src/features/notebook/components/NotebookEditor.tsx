...
const NotebookEditor = ({
  ...
  const { control, handleSubmit, reset } = useForm<NotebookEditorFormData>({
    defaultValues: {
      name: '',
      description: '',
      custom_prompt: '',
  ...
  const documentOptions = availableDocuments.map(
    (doc): EnhancedSelectOption & { document?: Document; [key: string]: unknown } => {
      ...
      return {
        ...
        tag: doc.status
          ...
          : undefined,
        searchableContent:
          `${doc.title} ${doc.filename || ''} ${(doc.metadata?.ocr_text as string) || ''}`.toLowerCase(),
        document: doc,
      };
    }
  );
  ...
  const handleDocumentSelectChange = (selectedOptions: unknown): void => {
    ...
    const documents = options
      ? options.map((option) => option.document).filter((doc): doc is Document => !!doc)
      : [];
          ...
          <div className="notebook-creator-content">
            ...
            <div className="form-section">
              <Textarea
                ...
                rules={{
                  maxLength: {
                    value: 1000,
                    message: 'Anweisungen dürfen maximal 1000 Zeichen haben',
                  },
                }}
              />
            </div>
            ...
            <div className="form-section">
              <div className="content-selection-header">
                <label className="form-label">Inhalte auswählen *</label>
                ...
                <div className="wolke-selection">
                  <WolkeSelector
                    ...
                    onChange={(selectedLinks: WolkeShareLink[]): void =>
                      setSelectedWolkeLinks(selectedLinks)
                    }
                    ...
                    scope={
                      Array.isArray(user?.groups) && (user.groups as unknown[]).length > 0
                        ? 'personal'
                        : 'personal'
                    }
                    isMulti={true}
