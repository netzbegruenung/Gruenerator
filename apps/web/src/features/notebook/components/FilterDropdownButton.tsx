...
const FilterDropdownButton = ({
  ...
  disabled = false,
  className = '',
}: FilterDropdownButtonProps): JSX.Element | null => {
  ...
  const filterGroups = useMemo((): FilterGroup[] => {
    ...
    normalizedCollections.forEach((collection) => {
      ...
      const fields: FilterField[] = Object.entries(
  ...
  const handleChipClick = (
    collectionIdParam: string | undefined,
    field: string,
    value: string | { date_from?: string; date_to?: string } | null
  ): void => {
        ...
        <div className="notebook-filter-dropdown">
          ...
          <div className="notebook-filter-dropdown-content">
            {filterGroups.map((group, groupIndex) => {
              const isExpanded = group.collectionId
                ? expandedCollections[group.collectionId] === true
                : false;
              const hasActiveFilters = group.fields?.some((f: FilterField) => f.activeValue);

              return (
                <div
                  key={group.collectionId || groupIndex}
                  className="notebook-filter-dropdown-collection"
                >
                  <button
                    type="button"
                  ...
                  {isExpanded && (
                    <div className="notebook-filter-dropdown-collection-content">
                      {group.loading ? (
                        ...
                        group.fields.map(
                          ({ field, label, type, values, min, max, activeValue }: FilterField) => {
                            const dateActiveValue = activeValue as
                              | { date_from?: string; date_to?: string }
                              | undefined;
                            const arrayActiveValue = activeValue as string[] | undefined;
                            return (
                              <div
                                key={`${group.collectionId}-${field}`}
                                className="notebook-filter-group"
                              >
                                {type === 'date_range' ? (
                                  <DateRangeFilter
                                    label={label}
                                    ...
                                    onDateFromChange={(value: string | null) =>
                                      handleChipClick(group.collectionId, field, {
                                        date_from: value || undefined,
                                        date_to: dateActiveValue?.date_to,
                                      })
                                    }
                                    onDateToChange={(value: string | null) =>
                                      handleChipClick(group.collectionId, field, {
                                        date_from: dateActiveValue?.date_from,
                                    ...
                                    <div className="notebook-filter-chips">
                                      {values.map((item: string | FilterValueItem) => {
                                        ...
                                        const displayValue = isObject
                                          ? (item as FilterValueItem).value
                                          : (item as string);
                                        const count = isObject
                                          ? (item as FilterValueItem).count
                                          : null;
                                        ...
                                        return (
                                          ...
                                          >
                                            {isActive && (
                                              <HiCheck className="notebook-filter-chip-check" />
                                            )}
                                            <span>{displayValue}</span>
                                            {count !== null && count !== undefined && (
                                              <span className="notebook-filter-chip-count">
                                                ({count})
                                              </span>
                                            )}
                                          </button>
