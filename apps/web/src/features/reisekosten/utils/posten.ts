...
export function buildPosten(state: ReisekostenState, computed: ComputeResult): Posten[] {
  ...
  const posten: Posten[] = [];
  if (state.fahrt.bahn) posten.push({ name: 'Bahn', detail: '2. Klasse', betrag: eur(f.bahn), beleg: belegState(state.fahrt.bahn) });
  if (state.fahrt.oepnv) posten.push({ name: 'ÖPNV', detail: 'Nahverkehr', betrag: eur(f.oepnv), beleg: belegState(state.fahrt.oepnv) });
  if (state.fahrt.kfz) posten.push({ name: 'Kfz', detail: `${state.fahrt.kfz.km} km`, betrag: eur(f.kfz) });
  if (state.fahrt.miete)
    posten.push({ name: 'Mietwagen / Carsharing', detail: 'lt. Beleg', betrag: eur(f.miete), beleg: belegState(state.fahrt.miete) });
  if (state.fahrt.taxi)
    posten.push({
      name: 'Taxi',
