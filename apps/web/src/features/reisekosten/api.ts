...
export async function extractBeleg(file: File, belegType: BelegTyp): Promise<ExtractBelegResponse> {
  ...
  const res = await getContractsClient().reisekosten.extractBeleg({
    body: { base64, filename: file.name, mimeType: file.type || 'application/octet-stream', belegType },
  });
...
export async function generatePdf(state: ReisekostenState): Promise<{ filename: string; blob: Blob }> {
