...
const WolkeShareLinkManager: React.FC<WolkeShareLinkManagerProps> = ({
  ...
  const shareLinks =
    !useStore || propShareLinks !== undefined ? propShareLinks || [] : storeShareLinks;
  ...
  const handleTestConnection = async (shareLink: ShareLink) => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        const message =
          'Verbindungstest fehlgeschlagen: ' + (result?.message || 'Unbekannter Fehler');
        if (useStore && !propOnErrorMessage) {
          setError(message);
        } else if (propOnErrorMessage) {
...
✅ Wenn Sie diese Datei sehen können, funktioniert die Verbindung korrekt!`;
  ...
  return (
    <div className="wolke-share-manager-container">
      ...
      <ProfileCard
        title={
          scope === 'group' ? 'Wolke-Link zur Gruppe hinzufügen' : 'Wolke-Verbindung hinzufügen'
        }
      >
        <div className="wolke-add-form-content">
          <p className="wolke-form-description">
            ...
            Eine detaillierte Schritt-für-Schritt Anleitung zum Erstellen eines Share-Links findest
            du{' '}
            <a
              ...
              <div className="wolke-form-group">
                ...
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setNewShareLink(e.target.value)
                  }
      ...
      {shareLinks.length > 0 && (
        <ProfileCard
          ...
          headerActions={
            ...
            >
              {React.createElement(getIcon('actions', 'refresh') as React.ElementType, {
                className: 'pabtn__icon',
              })}
            ...
            {shareLinks.map((shareLink: ShareLink | any) => (
              <div key={shareLink.id} className="wolke-share-link-item">
                <div className="wolke-share-link-info">
                  <div className="wolke-share-link-header">
                    ...
                    <div className="wolke-share-link-status">
                      <span
                        className={`status-badge ${shareLink.is_active ? 'active' : 'inactive'}`}
                      >
                ...
                <div className="wolke-share-link-actions">
                  ...
                  >
                    {React.createElement(getIcon('actions', 'refresh') as React.ElementType, {
                      className: `pabtn__icon ${testingConnections.has(shareLink.id) ? 'spinning' : ''}`,
                    })}
                  </button>
                  ...
                  >
                    {React.createElement(getIcon('actions', 'upload') as React.ElementType, {
                      className: `pabtn__icon ${uploadingFiles.has(shareLink.id) ? 'spinning' : ''}`,
                    })}
                  </button>
                  ...
                  <button
                    ...
                    title="Link kopieren"
                  >
                    {React.createElement(getIcon('actions', 'copy') as React.ElementType, {
                      className: 'pabtn__icon',
                    })}
                  </button>
                  ...
                  >
                    {React.createElement(getIcon('actions', 'share') as React.ElementType, {
                      className: 'pabtn__icon',
                    })}
                  </button>
                  ...
                  {canDeleteLinks && (
                    <button
                      ...
                      title="Löschen"
                    >
                      {React.createElement(getIcon('actions', 'delete') as React.ElementType, {
                        className: 'pabtn__icon',
                      })}
                    </button>
                  )}
