...
export const usePlanModeWorkflow = () => {
  ...
  const initiatePlan = useCallback(async (requestData: InitiateRequestData) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const hasQuestions =
          response.data.needsQuestions && mappedQuestions && mappedQuestions.length > 0;
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Fehler beim Generieren des Plans';
      setState((prev) => ({
        ...prev,
        status: 'error',
        error: errorMessage,
  ...
  const submitAnswers = useCallback(
    async (workflowId: string, answers: Record<string, string | string[]>) => {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Fehler beim Verfeinern des Plans';
        setState((prev) => ({
          ...prev,
          status: 'error',
          error: errorMessage,
  ...
  const generateProduction = useCallback(async (workflowId: string) => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Fehler beim Generieren des Inhalts';
      setState((prev) => ({
        ...prev,
        status: 'error',
        error: errorMessage,
  ...
  const submitCorrections = useCallback(async (workflowId: string, corrections: string) => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Fehler beim Anwenden der Korrekturen';
      setState((prev) => ({
        ...prev,
        status: 'error',
        error: errorMessage,
