...
const UNIVERSAL_OPTIONS: { value: UniversalSubType; label: string; icon: { category: string; name: string } }[] = [
  { value: 'rede', label: 'Rede', icon: { category: 'textTypes', name: 'rede' } },
  { value: 'wahlprogramm', label: 'Wahlprogramm', icon: { category: 'textTypes', name: 'wahlprogramm' } },
  { value: 'buergeranfragen', label: 'Bürger*innenanfragen', icon: { category: 'textTypes', name: 'buergeranfragen' } },
  { value: 'leichte_sprache', label: 'Leichte Sprache', icon: { category: 'accessibility', name: 'leichteSprache' } }
];
...
const TabSelector: React.FC<TabSelectorProps> = memo(({
  ...
  const handleUniversalOptionSelect = useCallback((subType: UniversalSubType) => {
  ...
  const handleDropdownKeyDown = useCallback((event: React.KeyboardEvent, subType: UniversalSubType) => {
    if (event.key === 'Enter' || event.key === ' ') {
      event.preventDefault();
