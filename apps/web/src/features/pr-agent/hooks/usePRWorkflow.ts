...
export const usePRWorkflow = () => {
  ...
  const generateStrategy = useCallback(
    async (requestData: {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Fehler beim Generieren der Strategie';
        setState((prev) => ({
          ...prev,
          status: 'error',
          error: errorMessage,
  ...
  const generateProduction = useCallback(
    async (workflowId: string, approvedPlatforms: string[], userFeedback?: string) => {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Fehler beim Generieren der Inhalte';
        setState((prev) => ({
          ...prev,
          status: 'error',
          error: errorMessage,
