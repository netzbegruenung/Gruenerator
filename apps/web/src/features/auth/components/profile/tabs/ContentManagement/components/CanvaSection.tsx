...
import {
  ...
  useCanvaConnection,
  useCanvaDesigns,
  useSavingDesignState,
} from '../../../../../../../stores/canvaStore';
...
const EMPTY_STATE_CONNECTED = {
  noDocuments:
    'Du hast noch keine Vorlagen. Verwende den "Sync mit Canva" Button um deine Canva-Designs zu laden.',
...
const CanvaSection = memo(
  ({
    ...
    const { currentTab: currentSubsection, handleTabClick: handleSubsectionClick } =
      useTabNavigation(
        initialSubsection,
    ...
    const {
      ...
      loading: fetchingCanvaDesigns,
      error: canvaDesignsError,
    } = useCanvaDesigns();
    ...
    const checkCanvaConnectionStatusRef = useRef<
      (() => Promise<CanvaConnectionResult | undefined>) | undefined
    >(undefined);
    ...
    const handleSaveCanvaTemplate = useCallback(
      async (canvaDesign: CanvaTemplate & { canva_id?: string }) => {
        try {
    ...
    const handleDeleteTemplate = async (templateId: string): Promise<void> => {
      try {
        ...
        onErrorMessage(
          'Fehler beim Löschen der Canva Vorlage: ' +
    ...
    const handleTemplateTitleUpdate = async (
      templateId: string,
      newTitle: string
    ): Promise<void> => {
      try {
        ...
        onErrorMessage(
          'Fehler beim Aktualisieren des Canva Vorlagentitels: ' +
            (error instanceof Error ? error.message : 'Unbekannter Fehler')
        );
        throw error;
      }
    ...
    const handleCreateAltText = useCallback(
    ...
    const getCanvaTemplateActionItems = useCallback(
      (template: CanvaTemplate): ActionItem[] => {
    ...
    const renderTemplateMetadata = (template: CanvaTemplate) => {
      const config = canvaUtils.getTemplateMetadataConfig(
        template,
        savedCanvaDesigns
      ) as TemplateMetadataConfig;
    ...
    const createShareAction = useCallback(
      (contentType: string) => {
        if (!onShareToGroup) {
          return () => {};
        }
      ...
      if (!canvaConnected) {
        if (currentSubsection === 'vorlagen' || currentSubsection === 'assets') {
          ...
          announceToScreenReader(
            'Zurück zur Übersicht - Canva-Verbindung erforderlich für diese Funktion'
          );
        }
      }
      ...
      <ProfileCard title={featureName}>
        <div className="login-required-card">
          ...
          <p className="login-required-message">
            Diese Canva-Funktionen stehen nur angemeldeten Nutzer*innen zur Verfügung. Bitte melde
            dich an, um deine Canva-Integration zu verwalten.
          </p>
    ...
    const searchPlaceholder = useMemo(
      () =>
        canvaConnected
          ? 'Alle Vorlagen und Canva-Designs durchsuchen...'
    ...
    const handleVorlagenDelete = useCallback(
      async (id: string) => {
    ...
    const handleVorlagenBulkDelete = useCallback(
      async (ids: string[]) => {
    ...
    const handleVorlagenEdit = useCallback(
      (item: unknown) => {
    ...
    const handleVorlagenShare = useCallback(
      (item: unknown) => {
    ...
    const handleVorlagenActionItems = useCallback(
      (item: unknown) => {
    ...
    const handleVorlagenMetaRender = useCallback(
      (item: unknown) => {
    ...
    const handleSyncWithCanva = useCallback(async () => {
      await Promise.all([getCanvaState().refreshDesigns(), templatesQuery.refetch()]);
