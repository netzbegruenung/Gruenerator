...
const EditableDetailForm = ({
  ...
  isLoading,
  disabled = false,
}: EditableDetailFormProps): React.ReactElement => {
...
const FormBuilderSection = ({
  ...
  updateFormSchema,
  disabled,
}: FormBuilderSectionProps): React.ReactElement => {
                ...
                <div key={optIndex} className="option-input-group">
                  <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                      ...
                      const currentAutoValue = (option.label || '')
                        .toLowerCase()
                        .replace(/\s+/g, '_')
                        .replace(/[^a-z0-9_]/g, '');
                      const isValueAutoGenerated =
                        option.value === currentAutoValue || !option.value;
