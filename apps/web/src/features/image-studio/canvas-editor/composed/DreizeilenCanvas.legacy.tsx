...
import { Group, Line, Text, Rect, Transformer, Layer } from 'react-konva';
...
import { AttributionOverlay } from '../primitives/AttributionOverlay';
...
export function DreizeilenCanvas({
  ...
  const {
    ...
    handleSave,
    getSnapTargets,
  } = useCanvasInteractions<SelectedElement>({
    stageRef,
  ...
  const handleRestore = useCallback((state: Record<string, unknown>) => {
    ...
    if (state.imageAttribution !== undefined) setImageAttribution(state.imageAttribution as StockImageAttribution | null);
  ...
  const collectState = useCallback(() => {
    ...
    balkenOffset, balkenScale, balkenWidthScale, barOffsets, currentImageSrc, imageAttribution,
  ...
  const handleDownload = useCallback(() => {
    if (exportedImage) {
      downloadCanvasImage(exportedImage, 'dreizeilen');
    }
  }, [exportedImage]);
  ...
  const handleShareExport = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (dataUrl) {
        ...
        // Don't call onExport anymore - we handle share ourselves
      }
    }, 50);
  ...
  // Attribution overlay data (only calculated when exporting)
  ...
  }, [isExporting, imageAttribution, config.canvas.width, config.canvas.height]);

  // Handler for canvas capture (used by share sidebar section)
  ...
  return (
    ...
    <GenericCanvasEditor
      ...
      state={state}
      actions={actions}
      onExport={() => {}} // No-op - share is now in sidebar
      onSave={handleSave}
      onAddPage={onAddPage}
      shareProps={{
        exportedImage,
    ...
    >
      <div className="dreizeilen-canvas-wrapper" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <CanvasStage ref={stageRef} width={config.canvas.width} height={config.canvas.height} responsive maxContainerWidth={maxWidth} onStageClick={(e) => { handleStageClick(e); setFocusedItemId(null); }} className="dreizeilen-stage">
          ...
          {/* Attribution overlay layer (only visible during export) */}
