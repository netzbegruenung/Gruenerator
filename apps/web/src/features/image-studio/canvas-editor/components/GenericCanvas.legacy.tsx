...
// ============================================================================
// CLIPBOARD (Global for session)
// ============================================================================
let genericCanvasClipboard: {
    type: 'balken' | 'shape' | 'illustration' | 'additional-text';
    data: any;
} | null = null;
...
export interface GenericCanvasProps<TState, TActions extends OptionalCanvasActions> {
    ...
    /** CSS class name */
    className?: string;
    /** Optional handler for adding a new page (multi-page mode) */
    onAddPage?: () => void;
    /** If true, render canvas without the layout wrapper (for multi-page stacking) */
    bare?: boolean;
    /** Optional handler for deleting this page (multi-page mode) - shows trash icon in floating bar */
    onDelete?: () => void;
}
...
function GenericCanvasInner<TState extends Record<string, any>, TActions extends OptionalCanvasActions>({
    ...
    }, [config.elements, setStateWrapper, saveToHistory, state, layout]);
    ...
    const canvasItems = useMemo(() => {
        const items: { id: string; type: 'element' | 'balken' | 'icon' | 'shape' | 'additional-text' | 'illustration'; data?: any }[] = [];
        ...
        // 6. Illustrations
        if ((state as any).illustrationInstances) {
            (state as any).illustrationInstances.forEach((i: any) => items.push({ id: i.id, type: 'illustration', data: i }));
        }
    ...
    // Delete key handler
    // Keyboard handlers (Delete, Copy, Paste)
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (isCtrlOrCmd && e.key === 'v') {
                if (!genericCanvasClipboard) return;

                const { type, data } = genericCanvasClipboard;
                ...
                setStateWrapper((prev) => {
                    ...
                    if (type === 'shape') {
                        ...
                        const newBalken = { ...data, id: newId, offset: { x: (data.offset?.x || 0) + offset, y: (data.offset?.y || 0) + offset } };
                ...
                // Save history
                // We utilize the fact that setStateWrapper triggers a render, but useCanvasHistorySetup
                // might need explicit save if we want it recorded immediately as a step.
                // Since genericCanvasClipboard.data is a snapshop, it's fine.
                // Ideally we should call saveToHistory inside setStateWrapper callback or equivalent, 
                // but setStateWrapper is just state update. We need to trigger history save manually or rely on effect?
                // GenericCanvas doesn't automatically save on all state changes unless actions do it.
                // We should probably call saveToHistory manually here, but we need the *next* state.
                // For now, let's rely on user interaction following paste to save, or trigger a save.
                // Actually, let's trigger it.
                // saveToHistory( ... ); // Implementation complicated because of async setState.
                // We'll leave explicit history save for now as this is a "batch" op.
            ...
            if (isCtrlOrCmd && e.key === 'c') {
                // Check what type the selected element is
                // 1. Shapes
                const shape = (state as any).shapeInstances?.find((s: any) => s.id === selectedElement);
                if (shape) {
                    genericCanvasClipboard = { type: 'shape', data: shape };
                    return;
                }
                ...
                if (ill) {
                    genericCanvasClipboard = { type: 'illustration', data: ill };
                    return;
                }
                ...
                if (balken) {
                    genericCanvasClipboard = { type: 'balken', data: balken };
                    return;
                }
                ...
                if (text) {
                    genericCanvasClipboard = { type: 'additional-text', data: text };
                    return;
                }
            ...
            if (e.key === 'Delete' || e.key === 'Backspace') {
                ...
                // Remove Illustration
                if ((state as any).illustrationInstances?.find((i: any) => i.id === selectedElement) && (actions as any).removeIllustration) {
                    (actions as any).removeIllustration(selectedElement);
                    setSelectedElement(null);
                    return;
                }
    ...
    const activeFloatingModule = useMemo(() => {
        ...
        const illustration = (state as any).illustrationInstances?.find((i: IllustrationInstance) => i.id === selectedElement);
        if (illustration) {
            return {
                type: 'illustration',
                data: illustration,
    ...
    const handleColorSelect = useCallback((color: string) => {
        ...
        if (activeFloatingModule.type === 'shape') {
            ...
            if ((actions as any).updateIllustration) {
                (actions as any).updateIllustration(activeFloatingModule.data.id, { color });
            }
    ...
    const handleOpacityChange = useCallback((id: string, opacity: number, type: 'shape' | 'icon' | 'text' | 'image' | 'illustration') => {
        ...
        if (type === 'shape') {
            ...
            if ((actions as any).updateIllustration) {
                (actions as any).updateIllustration(id, { opacity });
            }
        } else if (type === 'icon') {
            if ((actions as any).updateIcon) {
    ...
    // Content that goes inside the editor (or is rendered directly in bare mode)
    const canvasContent = (
        <div className={`generic-canvas-content ${className || ''}`}>
            ...
            <FloatingTapBar visible={true}>
                ...
                {activeFloatingModule && (
                    <>
                        ...
                        {(activeFloatingModule.type === 'shape' || activeFloatingModule.type === 'icon' || activeFloatingModule.type === 'illustration') && (
                            <>
                                <FloatingColorPicker
                                    currentColor={
                                ...
                                <FloatingOpacityControl
                                    opacity={activeFloatingModule.data.opacity ?? 1}
                                    onOpacityChange={(val) => handleOpacityChange(activeFloatingModule.data.id, val, activeFloatingModule.type as 'shape' | 'icon' | 'illustration')}
                                />
                            </>
                        )}
                    </>
                    ...
                    if (item.type === 'illustration') {
                        ...
                        return (
                            <IllustrationPrimitive
                                ...
                                onSelect={() => handleElementSelect(ill.id)}
                                onDragEnd={(x: number, y: number) => {
                                    if ((actions as any).updateIllustration) {
                                        (actions as any).updateIllustration(ill.id, { x, y });
                                    }
                                }}
                                onTransformEnd={(x: number, y: number, scale: number, rotation: number) => {
                                    if ((actions as any).updateIllustration) {
                                        (actions as any).updateIllustration(ill.id, { x, y, scale, rotation });
                                    }
                                }}
                                onSnapChange={handleSnapChange}
                                onSnapLinesChange={setSnapLines}
            ...
            {/* Add Page Button (when multi-page is enabled) */}
            {onAddPage && (
                <button
                    ...
                    onMouseOver={(e) => {
                        e.currentTarget.style.borderColor = 'var(--primary-500, #46962b)';
                        e.currentTarget.style.background = 'var(--primary-50, #f0f9ed)';
                        e.currentTarget.style.color = 'var(--primary-700, #2d6618)';
                    }}
                    onMouseOut={(e) => {
    ...
    // In bare mode, return just the canvas content without the layout wrapper
    if (bare) {
        return canvasContent;
    }

    // Normal mode: wrap with the full editor layout
    return (
        <GenericCanvasEditor
            config={config}
            state={canvasState}
