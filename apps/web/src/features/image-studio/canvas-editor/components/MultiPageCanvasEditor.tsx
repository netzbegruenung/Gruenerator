...
export function MultiPageCanvasEditor<TState extends Record<string, any>, TActions>({
    ...
    const handlePageStateChange = useCallback((pageId: string, partial: Partial<TState>) => {
        setPages(prev =>
            prev.map(p =>
                p.id === pageId ? { ...p, state: { ...p.state, ...partial } } : p
            )
        );
    }, []);

    // Export single page
    const handleExportPage = useCallback((base64: string, pageIndex: number) => {
                ...
                {sortedPages.map((page, index) => (
                    <div key={page.id} className="page-wrapper">
                        ...
                        <GenericCanvas
                            ...
                            onAddPage={index === sortedPages.length - 1 && canAddMorePages ? handleAddPage : undefined}
                        />
                    </div>
                ))}
            </div>
