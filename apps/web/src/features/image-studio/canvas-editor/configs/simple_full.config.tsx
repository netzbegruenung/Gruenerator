...
import { type ShapeInstance, type ShapeType, createShape } from '../utils/shapes';
...
export const simpleFullConfig: FullCanvasConfig<SimpleFullState, SimpleFullActions> = {
  ...
  sections: {
    text: {
      ...
      propsFactory: (state, actions) => ({
        ...
        onUpdateAdditionalTextFontSize: (id: string, size: number) =>
  ...
  elements: [
    ...
    },
    {
      ...
      fill: (state: SimpleFullState, _layout: LayoutResult) =>
        state.headlineColor ?? SIMPLE_CONFIG.headline.color,
    ...
    },
    {
      ...
      fill: (state: SimpleFullState, _layout: LayoutResult) =>
        state.subtextColor ?? SIMPLE_CONFIG.subtext.color,
  ...
  createInitialState: (props: Record<string, unknown>) => ({
    ...
    alternatives:
      (props.alternatives as Array<Record<string, unknown>> | undefined)?.map(
        (a: Record<string, unknown>) => String(a.headline || '')
      ) ?? [],
  ...
  createActions: (getState, setState, saveToHistory, debouncedSaveToHistory, callbacks) => ({
    ...
    addShape: (type: ShapeType) => {
      ...
      const newShape = createShape(type, x, y, '#005538'); // Default to primary green
      setState((prev) => ({
        ...prev,
