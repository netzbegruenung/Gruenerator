import { Check, Users, X } from 'lucide-react';
import { memo } from 'react';

import {
  getNotificationActions,
  truncateBody,
  type NotificationActionContext,
} from '../notificationConfig';

import type { Notification } from '../types';

import { cn } from '@/utils/cn';

interface NotificationGroupProps {
  items: Notification[];
  formatTime: (date: string) => string;
  onMarkAsRead: (id: string) => void;
  onDismiss: (id: string, isUnread: boolean) => void;
  navigate: (path: string) => void;
  refreshProfile: () => void;
}

const MAX_VISIBLE_ITEMS = 5;

function extractGroupName(items: Notification[]): string {
  for (const item of items) {
    const name = item.metadata?.groupName;
    if (typeof name === 'string') return name;
  }
  return 'Gruppe';
}

const NotificationGroup = memo(
  ({
    items,
    formatTime,
    onMarkAsRead,
    onDismiss,
    navigate,
    refreshProfile,
  }: NotificationGroupProps) => {
    const hasUnread = items.some((n) => !n.is_read);
    const mostRecent = items[0];
    const groupName = extractGroupName(items);
    const visibleItems = items.slice(0, MAX_VISIBLE_ITEMS);
    const remaining = items.length - MAX_VISIBLE_ITEMS;

    const ctx: NotificationActionContext = {
      notification: mostRecent,
      navigate,
      markAsRead: onMarkAsRead,
      refreshProfile,
    };
    const actions = getNotificationActions(mostRecent.type, ctx);

    const handleMarkAllRead = () => {
      items.filter((n) => !n.is_read).forEach((n) => onMarkAsRead(n.id));
    };

    const handleDismissAll = () => {
      items.forEach((n) => onDismiss(n.id, !n.is_read));
    };

    return (
      <div
        className={cn(
          'group/notif flex gap-sm px-md py-xs transition-colors',
          hasUnread && 'bg-primary-50 dark:bg-primary-900/10'
        )}
      >
        <div className="flex flex-col items-center justify-center gap-xxs shrink-0 self-center">
          <div className="flex items-center justify-center size-8 rounded-full bg-grey-100 dark:bg-grey-800">
            <Users className="size-4 text-grey-500" />
          </div>
          <span className="text-[0.6rem] text-grey-400 leading-none">
            {formatTime(mostRecent.created_at)}
          </span>
        </div>

        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-xs">
            <div className="min-w-0">
              <div
                className={cn(
                  'text-[0.8rem] leading-snug',
                  hasUnread ? 'font-semibold' : 'font-normal'
                )}
              >
                {groupName}
                <span className="text-grey-400 font-normal ml-xs">({items.length})</span>
              </div>
              <div className="flex flex-col gap-0 mt-xxs">
                {visibleItems.map((item) => (
                  <p
                    key={item.id}
                    className="text-[0.7rem] leading-relaxed text-foreground m-0 truncate"
                  >
                    <span className="text-grey-400 mr-xxs">·</span>
                    {truncateBody(item.body) || item.title}
                  </p>
                ))}
                {remaining > 0 && (
                  <p className="text-[0.7rem] text-grey-400 m-0">+{remaining} weitere</p>
                )}
              </div>
            </div>

            <div className="shrink-0 hidden group-hover/notif:flex items-center gap-xxs">
              {hasUnread && (
                <button
                  type="button"
                  className="flex items-center justify-center size-4 rounded-full text-grey-400 hover:text-primary-500 transition-colors"
                  onClick={(e) => {
                    e.stopPropagation();
                    handleMarkAllRead();
                  }}
                  aria-label="Alle gelesen"
                  title="Alle gelesen"
                >
                  <Check className="size-3" />
                </button>
              )}
              <button
                type="button"
                className="flex items-center justify-center size-4 rounded-full text-grey-400 hover:text-foreground transition-colors"
                onClick={(e) => {
                  e.stopPropagation();
                  handleDismissAll();
                }}
                aria-label="Erledigt"
                title="Erledigt"
              >
                <X className="size-3" />
              </button>
            </div>
          </div>

          {actions.length > 0 && (
            <div className="flex flex-wrap gap-xs mt-xs">
              {actions.map((action) => (
                <button
                  key={action.label}
                  type="button"
                  onClick={(e) => {
                    e.stopPropagation();
                    action.run(ctx);
                  }}
                  className="flex items-center gap-xxs rounded-full border border-grey-200 dark:border-grey-700 px-xs py-0 text-[0.7rem] leading-relaxed text-foreground hover:border-primary-500 hover:text-primary-500 transition-colors"
                >
                  <action.icon className="size-3" />
                  {action.label}
                </button>
              ))}
            </div>
          )}
        </div>
      </div>
    );
  }
);

NotificationGroup.displayName = 'NotificationGroup';

export default NotificationGroup;
