import { JSX, useCallback, KeyboardEvent } from 'react';
...
const CitationSourcesDisplay = ({
  ...
  const handleCitationClick = useCallback((citation: Citation) => {
    if (citation.document_id && citation.chunk_index !== undefined) {
      fetchChunkContext(citation.document_id, citation.chunk_index, citation, linkConfig);
    }
  }, [fetchChunkContext, linkConfig]);

  const handleCitationKeyDown = useCallback((e: KeyboardEvent<HTMLDivElement>, citation: Citation) => {
          ...
          <div key={group.documentId || group.documentTitle || `doc-${index}`} className="ask-document-group">
            <div className="ask-document-header">
              <h5
                ...
                onClick={() => canLink && (group.documentId || group.url) && handleDocumentClick(group.documentId, group.url ?? null)}
              >
                {group.documentTitle}
              </h5>
              {group.relevance && (
                ...
                {group.citations.map((citation: Citation, idx: number) => {
                  ...
                  return (
                    <div
                      ...
                      onKeyDown={isClickable ? (e) => handleCitationKeyDown(e, citation) : undefined}
              ...
              <div key={source.document_id || source.document_title || `additional-${idx}`} className="ask-additional-source-item">
                <div className="ask-additional-source-header">
                  <span
                    ...
                    onClick={() => canLink && (source.document_id || source.url) && handleDocumentClick(source.document_id, source.url ?? null)}
                  >
