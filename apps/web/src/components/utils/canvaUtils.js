...
export const initiateCanvaLogin = async (onError) => {
  try {
    ...
    const errorMessage =
      'Fehler beim Verbinden mit Canva: ' + (error.message || 'Bitte versuche es später erneut.');
...
export const saveCanvaTemplate = async (canvaDesign, onSuccess = null, onError = null) => {
  ...
  try {
    ...
    const result = await templateService.createUserTemplateFromUrl(canvaDesign.canva_url, true);
...
export const getCanvaOverviewStats = async (isAuthenticated, canvaConnected, canvaUser = null) => {
  ...
  try {
    ...
    const designCount =
      designsResponse.status === 'fulfilled' && designsResponse.value.data.success
        ? designsResponse.value.data.total_count || 0
        : 0;

    const assetCount =
      assetsResponse.status === 'fulfilled' && assetsResponse.value.data.success
        ? assetsResponse.value.data.total_count || 0
        : 0;
 ...
 */
export const uploadAssetToCanva = async (
  file,
  name,
  tags = [],
  onProgress = null,
  ...
  try {
    ...
    const errorMessage =
      'Fehler beim Hochladen des Assets: ' + (error.message || 'Unbekannter Fehler');
    onError?.(errorMessage);
    throw new Error(errorMessage);
  }
};
...
export const uploadAssetFromUrlToCanva = async (url, name, tags = [], onError = null) => {
  try {
    ...
    const errorMessage =
      'Fehler beim Hochladen des Assets von URL: ' + (error.message || 'Unbekannter Fehler');
    onError?.(errorMessage);
    throw new Error(errorMessage);
  }
};
...
export const getAssetUploadJobStatus = async (jobId, onError = null) => {
  try {
    ...
    const errorMessage =
      'Fehler beim Abrufen des Upload-Status: ' + (error.message || 'Unbekannter Fehler');
    onError?.(errorMessage);
    throw new Error(errorMessage);
  }
};
...
export const deleteCanvaAsset = async (assetId, onSuccess = null, onError = null) => {
  try {
    ...
    const errorMessage =
      'Fehler beim Löschen des Assets: ' + (error.message || 'Unbekannter Fehler');
    onError?.(errorMessage);
    throw new Error(errorMessage);
  }
};
 ...
 */
export const updateCanvaAssetMetadata = async (
  assetId,
  metadata,
  onSuccess = null,
  onError = null
  ...
  try {
    ...
    const errorMessage =
      'Fehler beim Aktualisieren der Asset-Metadaten: ' + (error.message || 'Unbekannter Fehler');
    onError?.(errorMessage);
    throw new Error(errorMessage);
  }
};
 ...
 */
export const importAssetPackage = async (
  packageId,
  onProgress = null,
  onComplete = null,
  onError = null
