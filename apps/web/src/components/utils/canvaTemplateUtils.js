...
export const generateCanvaTemplateActionItems = (template, options) => {
  const { onEditTemplate, onDeleteTemplate, onShareToGroup, onCreateAltText, onErrorMessage } =
    options;
...
export const handleBulkDeleteTemplates = async (templateIds, onErrorMessage) => {
  try {
    ...
    if (canvaDesignIds.length > 0) {
      onErrorMessage(
        `${canvaDesignIds.length} Canva Designs können nicht gelöscht werden. Sie können nur in Canva selbst gelöscht werden.`
      );
    }
...
export const formatTemplateForDisplay = (template) => {
  return {
    ...
    displayDate: template.created_at
