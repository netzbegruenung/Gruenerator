...
/**
 * Reference to the fallback entry — callers that only want to toast
 * *specific* messages (e.g. `toastApiError`) compare against this to skip
 * the unclassified case rather than surface a vague "Unerwarteter Fehler".
 */
...
export const getErrorMessage = (error: ErrorInput): ErrorMessageInfo => {
  ...
  if (typeof error === 'string' && error.includes('Something broke!')) {
    return {
      title: 'Serverfehler',
      message:
        'Ein unerwarteter Fehler ist auf dem Server aufgetreten. Bitte versuchen Sie es später erneut.',
