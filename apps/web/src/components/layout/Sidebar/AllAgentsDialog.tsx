import { agentsList } from '@gruenerator/chat';
...
export function AllAgentsDialog({ onLinkClick, titleClass }: AllAgentsDialogProps) {
  ...
  const visibleSkills = useMemo(
    () => agentsList.filter((s) => !HIDDEN_INVENTORY_MENTIONS.has(s.mention ?? '')),
    []
  );
        ...
        <ul className="list-none m-0 p-0 max-h-[60vh] overflow-y-auto scrollbar-thin">
          ...
          {visibleSkills.map((skill) => (
            <AgentListRow
              ...
              star={{
                active: favoritesSet.has(skill.mention),
                onToggle: () => toggle(skill.mention),
              }}
            />
          ))}
            ...
            <>
              ...
              {userAgents.map((agent) => (
                <AgentListRow
                  ...
                  star={{
                    active: favoritesSet.has(agent.identifier),
