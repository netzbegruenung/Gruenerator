...
export const GanttContentHeader: FC<GanttContentHeaderProps> = ({
  ...
  return (
    ...
    >
      <div>
        <div
          className="sticky inline-flex whitespace-nowrap px-3 py-2 text-grey-400 text-xs"
          style={{
      ...
      >
        {Array.from({ length: columns }).map((_, index) => (
          <div
            className="shrink-0 border-grey-200/50 dark:border-grey-700/50 border-b py-1 text-center text-xs"
            key={`${id}-${index}`}
          >
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({
  ...
  return (
    <div
      className={cn(
        'relative flex items-center gap-2.5 p-2.5 text-xs hover:bg-grey-50 dark:bg-[#1e1e1e]',
        className
      )}
      key={feature.id}
      onClick={handleClick}
    ...
    >
      ...
      <p className="pointer-events-none text-grey-400">{duration}</p>
    </div>
  );
};
...
export const GanttSidebarHeader: FC = () => (
  <div
    className="sticky top-0 z-10 flex shrink-0 items-end justify-between gap-2.5 border-grey-200/50 dark:border-grey-700/50 border-b bg-background/90 p-2.5 font-medium text-grey-400 text-xs backdrop-blur-sm"
    style={{ height: 'var(--gantt-header-height)' }}
  >
...
export const GanttSidebarGroup: FC<GanttSidebarGroupProps> = ({ children, name, className }) => (
  <div className={className}>
    <p
      className="w-full truncate p-2.5 text-left font-medium text-grey-400 text-xs"
  ...
  return (
    ...
    >
      <button
        ...
        type="button"
      >
        <PlusIcon className="pointer-events-none select-none text-grey-400" size={16} />
      </button>
    </div>
  );
};
  ...
  return (
    ...
    <div
      className={cn(
        ...
        isColumnSecondary?.(index) ? 'bg-grey-50 dark:bg-[#1e1e1e]' : ''
      )}
    ...
    >
      ...
      >
        <button
          ...
          type="button"
        >
          <PlusIcon className="text-grey-400" size={12} />
        </button>
        <div className="whitespace-nowrap rounded-full border border-grey-200/50 dark:border-grey-700/50 bg-background/90 px-2 py-1 text-foreground text-xs backdrop-blur-lg">
  ...
  return (
    ...
    >
      ...
      {date && (
        <div
          className={cn(
            '-translate-x-1/2 absolute top-10 hidden whitespace-nowrap rounded-lg border border-grey-200/50 dark:border-grey-700/50 bg-background/90 px-2 py-1 text-foreground text-xs backdrop-blur-lg group-hover:block',
            isPressed && 'block'
          )}
        >
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  return (
    ...
    >
      <div
        className={cn(
          'gantt relative isolate grid h-full w-full flex-none select-none overflow-auto rounded-sm bg-grey-50 dark:bg-[#1e1e1e]',
          range,
          className
        )}
        ref={scrollRef}
