...
const OutOfBoundsDay = ({ day }: OutOfBoundsDayProps) => (
  <div className="relative h-full w-full bg-grey-50 dark:bg-[#1e1e1e] p-1 text-grey-400 text-xs">
    {day}
  </div>
);
...
export const CalendarBody = ({ features, children }: CalendarBodyProps) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      <div
        className="relative flex h-full w-full flex-col gap-1 p-1 text-grey-400 text-xs"
        key={day}
      >
        {day}
        <div>{featuresForDay.slice(0, 3).map((feature) => children({ feature }))}</div>
        {featuresForDay.length > 3 && (
          <span className="block text-grey-400 text-xs">+{featuresForDay.length - 3} more</span>
        )}
      </div>
    );
  }
...
export const CalendarHeader = ({ className }: CalendarHeaderProps) => {
  ...
  return (
    <div className={cn('grid flex-grow grid-cols-7', className)}>
      {daysData.map((day) => (
        <div className="p-3 text-right text-grey-400 text-xs" key={day}>
          {day}
        </div>
      ))}
    </div>
