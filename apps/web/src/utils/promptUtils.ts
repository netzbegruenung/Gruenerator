...
interface FormData {
  ...
  platforms?: string[];
  zitatgeber?: string;
  pressekontakt?: string;
  [key: string]: unknown;
}

// Function to safely render Markdown to HTML using backend service
export const renderMarkdown = async (markdown: string | null | undefined): Promise<HtmlOutput> => {
 ...
 * @param customInstructions User's custom instructions/prompt.
 * @param knowledgeContent The formatted string of selected knowledge items.
 * @param basePrompt Additional base prompt/context.
 * @returns The structured prompt string, or null if all inputs are empty.
 */
export const createStructuredFinalPrompt = (
  customInstructions: string | null | undefined,
  knowledgeContent: string | null | undefined,
  basePrompt: string | null | undefined = null
): string | null => {
 ...
 * @param customInstructions User's custom instructions/prompt.
 * @param knowledgeContent The formatted string of selected knowledge items.
 * @param memoryContent The formatted string of relevant memories from mem0.
 * @param basePrompt Additional base prompt/context.
 * @param documentContent The formatted string of relevant document excerpts (from API search).
 * @param selectedDocumentContent The formatted string of user-selected documents.
 * @returns The structured prompt string, or null if all inputs are empty.
 */
export const createPromptWithMemories = (
  customInstructions: string | null | undefined,
  knowledgeContent: string | null | undefined,
  memoryContent: string | null | undefined,
  basePrompt: string | null | undefined = null,
  documentContent: string | null | undefined = null,
  selectedDocumentContent: string | null | undefined = null
): string | null => {
  const instructionsPart = customInstructions?.trim() || '';
  const knowledgePart = knowledgeContent?.trim() || '';
 ...
 * @returns Formatted base prompt
 */
export const createBasePromptFromFormData = (formData: FormData): string => {
 ...
 * @param basePrompt The user's custom prompt.
 * @param knowledgeString The formatted string of selected knowledge items.
 * @returns The combined prompt string, or null if both inputs are empty.
 * @deprecated Use createStructuredFinalPrompt for better structure
 */
export const createFinalPrompt = (
  basePrompt: string | null | undefined,
  knowledgeString: string | null | undefined
): string | null => {
