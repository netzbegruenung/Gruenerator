...
export class NextcloudShareManager {
  ...
  static async updateShareLink(shareLinkId: string, updates: ShareLinkUpdate): Promise<ShareLink> {
    try {
      const response = await apiClient.put<ApiResponseData>(
        `/nextcloud/share-links/${shareLinkId}`,
        updates
      );
   ...
   */
  static async uploadTest(
    shareLinkId: string,
    content: string,
    filename = 'gruenerator-test.txt'
  ...
  static async getUsageStats(): Promise<UsageStats> {
    try {
      ...
      const oldestLinkTime =
        linksWithDates.length > 0
          ? Math.min(
              ...linksWithDates.map((link: ShareLink) => new Date(link.created_at!).getTime())
            )
          : null;
      const newestLinkTime =
        linksWithDates.length > 0
          ? Math.max(
              ...linksWithDates.map((link: ShareLink) => new Date(link.created_at!).getTime())
            )
