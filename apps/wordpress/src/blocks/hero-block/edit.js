...
const Edit = ({ attributes, setAttributes }) => {
  ...
  useEffect(() => {
    ...
    if (!heroText) {
      setAttributes({
        heroText: __('Kandidat*in für den Wahlkreis 54 Musterstadt-Musterort.', 'gruenerator'),
      });
    }
    if (!socialLinks) {
      setAttributes({
              ...
              <MediaUpload
                ...
                render={({ open }) => (
                  <div>
                    ...
                    >
                      {heroImageUrl
                        ? __('Bild ändern', 'gruenerator')
                        : __('Bild auswählen', 'gruenerator')}
                    </Button>
                  </div>
                )}
              />
        ...
        <div className="hero-block" style={{ width: '100%' }}>
          ...
          <div className="hero-right">
            ...
            <RichText
              ...
              placeholder={__(
                'Kandidat*in für den Wahlkreis 54 Musterstadt-Musterort.',
                'gruenerator'
              )}
            />
            <div className="social-icons">
              {socialLinks?.facebook && (
                <Button icon="facebook" href={socialLinks.facebook} target="_blank" />
              )}
              {socialLinks?.twitter && (
                <Button icon="twitter" href={socialLinks.twitter} target="_blank" />
              )}
              {socialLinks?.youtube && (
                <Button icon="youtube" href={socialLinks.youtube} target="_blank" />
              )}
            </div>
          </div>
        </div>
