...
export const useResolveUsers = () => {
  ...
  const resolveUsers = useCallback(async (userIds: string[]): Promise<(BlockNoteUser | undefined)[]> => {
    const now = Date.now();
    ...
    try {
      ...
      for (const dbUser of users) {
        const blockNoteUser: BlockNoteUser = {
          id: dbUser.id,
          name: dbUser.display_name || dbUser.email?.split('@')[0] || 'Unbekannt',
          avatarUrl: dbUser.avatar_url || `/avatars/robot-${dbUser.avatar_robot_id || 1}.svg`,
        };
