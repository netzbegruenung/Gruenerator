...
export type ToolKey = 'search' | 'web' | 'person' | 'examples' | 'research';
...
interface AgentState {
  ...
  loadCompactionState: (threadId: string) => Promise<void>;
  triggerCompaction: (threadId: string) => Promise<void>;
...
export const useAgentStore = create<AgentState>()(
  persist(
    (set, get) => ({
      ...
      loadCompactionState: async (threadId: string) => {
      ...
      triggerCompaction: async (threadId: string) => {
        set({ compactionLoading: true });
        try {
          ...
          if (response.success && !response.skipped) {
            ...
            console.log(`[Compaction] Summary generated for thread ${threadId}`);
          }
