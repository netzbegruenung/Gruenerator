...
export const ToolCallUI = memo(function ToolCallUI({ toolName, args, state, result }: ToolCallUIProps) {
  ...
  const query = useMemo(() => {
    // Handle both 'query' (standard tools) and 'question' (research tool)
  ...
  const resultCount = useMemo(() => {
    ...
    // Research tool uses citations array
...
// Research Result UI - Perplexity-style display with citations
interface Citation {
  id: number;
  title: string;
  url: string;
...
const ResearchResultUI = memo(function ResearchResultUI({ result }: { result: unknown }) {
  ...
  // Parse answer to highlight citation markers [1], [2], etc.
  ...
  return (
    <div className="space-y-3">
      {/* Confidence and search steps indicator */}
      <div className="flex items-center gap-2 text-[10px]">
        ...
        {searchSteps && searchSteps.length > 0 && (
          <span className="text-foreground-muted">
            · {searchSteps.length} Suche{searchSteps.length > 1 ? 'n' : ''}
          </span>
        )}
      </div>
